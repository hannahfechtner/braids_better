...
theorem count_down_first {i j : ℕ} (h : i ≤ j) : count_down (j+1) i =
                  ↑j * (count_down j i) := by
  unfold count_down
  rw [count_up_pop]
...
theorem sigma_bar_self : sigma_bar i i = 1 := by
  unfold sigma_bar
  simp

theorem sigma_bar_succ_ascending : sigma_bar i (i + 1) = of i := by
  unfold sigma_bar
  simp

theorem sigma_bar_succ_descending : sigma_bar (i + 1) i = of i := by
  unfold sigma_bar
  simp

theorem sigma_bar_ascending_first {i j : ℕ} (h: i < j) : sigma_bar i j =
    of i * (sigma_bar (i + 1) j) := by
  simp only [sigma_bar, mul_ite]
  split
  · next h1 =>
    split
    · next h3 =>
...
theorem sigma_bar_ascending_pop {i j : ℕ} (h: i < j) : sigma_bar i j = sigma_bar i (j - 1) *
    (of (j-1)) := by
  simp only [sigma_bar, mul_ite]
  split
  · next h1 =>
...
theorem sigma_bar_descending_first {i j : ℕ} (h: i ≥ j) :
    sigma_bar (i + 1) j = (of i) * sigma_bar i j := by
  simp only [sigma_bar, mul_ite]
  split
  · next h1 => linarith
  next h2 =>
...
theorem sigma_bar_descending_pop {i j : ℕ} (h: i<j) : sigma_bar j i = sigma_bar j (i + 1) *
...
theorem sigma_bar_ascending_bounded (n : ℕ) {k : ℕ}: k ∈ (sigma_bar n 0) → k < n := by
  intro k_in
