...
structure Equivalence_C {α : Type v} (r : α → α → Type u) : Type (max u v) where
...
-/
class Setoid_C (α : Type u) where
  /-- `x ≈ y` is the distinguished equivalence relation of a setoid. -/
  r : α → α → Type v
  iseqv : Equivalence_C r

instance {α : Type u} [Setoid_C α] : HasEquiv α :=
  ⟨Setoid_C.r⟩

namespace Setoid_C

variable {α : Type u} [Setoid_C α]
...
/-- The inductively defined smallest multiplicative congruence relation containing a given binary
    relation. -/
inductive ConGen.Rel [Mul M] (r : M → M → Type v) : M → M → Type (max u v)
...
/-- The inductively defined smallest multiplicative congruence relation containing a given binary
    relation. -/
def conGen [Mul M] (r : M → M → Type) : Con M :=
...
protected theorem symm {x y} : c x y → c y x := c.toSetoid_C.symm

/-- Congruence relations are transitive. -/
protected theorem trans {x y z} : c x y → c y z → c x z := c.toSetoid_C.trans

/-- Multiplicative congruence relations preserve multiplication. -/
protected theorem mul {w x y z} : c w x → c y z → c (w * y) (x * z) := c.mul'

theorem rel_mk {s : Setoid_C M} {h a b} : Con.mk s h a b ↔ r a b :=
  Iff.rfl
...
-- theorem toSetoid_inj {c d : Con M} (H : c.toSetoid_C = d.toSetoid_C) : c = d :=
