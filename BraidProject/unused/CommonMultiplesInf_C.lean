...
def multiple_delta_neg (u : FreeMonoid ℕ) (l n : ℕ) (h : FreeMonoid.length u <= l)
  (bounded : ∀ x, x ∈ u → x < n)
  : Σ w, PLift (BraidMonoidInf.mk (u * w) = BraidMonoidInf.mk ((delta_neg n)^l) ∧
    (∀ x, x ∈ w → x < n)) := by
  have helper : ∀ l, ∀ n, ∀ u, (u.length <= l) → (∀ x, x ∈ u → x < n) →
      Σ w, PLift (BraidMonoidInf.mk (u * w) = BraidMonoidInf.mk ((delta_neg n)^l) ∧ (∀ x, x ∈ w → x < n)) := by
    intro l
    induction l
...
def is_bounded (u : FreeMonoid ℕ) : Σ k, PLift (∀ x∈ u, x<k) := by
  sorry
...
def common_right_mul_inf_mk (u v : FreeMonoid ℕ) : Σ (u' v' : FreeMonoid ℕ), PLift (BraidMonoidInf.mk (u*v') = BraidMonoidInf.mk (v*u')) := by
  rcases (is_bounded u) with ⟨k1, hk1⟩
  rcases (is_bounded v) with ⟨k2, hk2⟩
  have u_under : ∀ x ∈u, x < Nat.max k1 k2 :=
    fun x h => Nat.lt_of_lt_of_le (hk1.1 x h) (Nat.le_max_left k1 k2)
  have v_under : ∀ x ∈v, x < Nat.max k1 k2 :=
    fun x h => Nat.lt_of_lt_of_le (hk2.1 x h) (Nat.le_max_right k1 k2)
...
-- def common_right_mul_inf (u v : PresentedMonoid (@braid_rels_m_inf)) :
--     Σ (u' v' : FreeMonoid ℕ ), PLift (((u * (PresentedMonoid.mk (braid_rels_m_inf) v'))) =
