...
def stable (u v : FreeMonoid ℕ) := ∀ a b, grid u v a b → ∀ u' v',
  ...
  BraidMonoidInf.mk v = BraidMonoidInf.mk v' → Σ a' b',
    grid u' v' a' b' × PLift (BraidMonoidInf.mk a = BraidMonoidInf.mk a') ×
    PLift (BraidMonoidInf.mk b = BraidMonoidInf.mk b')

def stable_far_apart (i j k : ℕ) (h : Nat.dist j k >= 2) :
  ...
  rcases BraidMonoidInf.pair_eq_C h v_eq with ⟨one, two, three, four⟩
  · use a, b
    exact ⟨grid_uvab, ⟨⟨by trivial⟩, ⟨by trivial⟩⟩⟩
  rename_i working_h
  rw [working_h.1]
  rcases splittable_vertically_of_grid grid_uvab (of j) (of k) rfl with ⟨u, d, f, h1, h2, ⟨rfl⟩⟩
  rcases trichotomous_dist_C i j
  · rcases trichotomous_dist_C i k
    · rename_i hij hik
      have ud := helpier_ij h1 _ _ (or_dist_iff.mp hij.1) rfl rfl
      rw [ud.1] at h2
      have af := helpier_ij h2 _ _ (or_dist_iff.mp hik.1) rfl rfl
      rw [ud.2, af.1, af.2]
      use of i, of k * of j
      exact ⟨grid.horizontal (grid.separated i k hik.1)
        (grid.separated i j hij.1),
    ...
    rcases last_two
    · rename_i one two
      have ud := helpier_ij h1 _ _ (or_dist_iff.mp one.1) rfl rfl
      rw [ud.1] at h2
      rw [ud.2]
      use of i * of k, of k * of i * of j
      constructor
      · rw [Nat.dist_comm] at h
        have H := (grid.vertical (grid.separated i j one.1)
          (grid.separated k j h))
        -- have H := (grid.vertical (grid.separated i j (or_dist_iff.mp one))
        --   (grid.separated k j (or_dist_iff.mp h)))
        exact grid.horizontal (grid.adjacent i k two.1) H
      have H := helpier_close h2 i k two.1 rfl rfl
      constructor
      · rw [H.1]
        exact ⟨rfl⟩
      rw [H.2]
      ...
      apply ConGen.Rel.mul
      · exact ConGen.Rel.refl _
      exact BraidMonoidInf.comm_rel (by rw [Nat.dist_comm] at one; exact one.1)
    rename_i hij hik
    have ud := helpier_ij h1 _ _ (or_dist_iff.mp hij.1) rfl rfl
    rw [ud.1, hik.1] at h2
    have af := helpier_eq h2 _ rfl rfl
    rw [af.1, af.2, hik.1, ud.2]
    use 1, of j
  ...
  · rcases trichotomous_dist_C i k
    · rename_i one two
      ...
      have H1 : PresentedMonoid.rel braid_rels_m_inf (of j * of i * of k) (of j * of k * of i) := by
        rw [mul_assoc, mul_assoc]
        apply ConGen.Rel.mul (ConGen.Rel.refl _) (BraidMonoidInf.comm_rel two.1)
      exact H1.trans <| ConGen.Rel.mul (BraidMonoidInf.comm_rel h) (ConGen.Rel.refl _)
    rename_i last_two
    rcases last_two
    · rename_i hij hik
      have ud := helpier_close h1 _ _ hij.1 rfl rfl
      rw [ud.2]
      rw [ud.1] at h2
      have H_split := splittable_horizontally_of_grid h2 (of i) (of j) rfl
      rcases H_split with ⟨u₁, c₁, c₂, ha, hb, hc⟩
      have c₁u₁ := helpier_close ha _ _ hik.1 rfl rfl
      rw [c₁u₁.2] at hb
      ...
      specialize c₂d₂ hij.1 rfl rfl
      use of i * of j * of k * of i, of k * of i * of j * of i * of k
      constructor
      · rw [Nat.dist_comm] at hij
        apply grid.horizontal (grid.adjacent i k hik.1) (grid.vertical (grid.adjacent i j hij.1)
          (grid.horizontal (grid.separated k j (by rw [Nat.dist_comm] at h; exact h))
          (grid.adjacent k i (by rw [Nat.dist_comm] at hik; exact hik.1))))
      ...
      have H1 : PresentedMonoid.rel braid_rels_m_inf (of j * of i * (of k * (of i * of j)))
        ...
        exact ConGen.Rel.mul (BraidMonoidInf.braid_rel hik.1) (ConGen.Rel.refl _)
      ...
      have H3 : PresentedMonoid.rel braid_rels_m_inf (of k * (of j * of i * of j) * of k)
        ...
        apply ConGen.Rel.mul (ConGen.Rel.mul (ConGen.Rel.refl _) (BraidMonoidInf.braid_rel hij.1))
          (ConGen.Rel.refl _)
      exact (H1.trans H2).trans H3
    rename_i one two
    rw [two.1, Nat.dist_comm k j] at one
    rw [one.1] at h
    linarith [h]
  rcases trichotomous_dist_C i k
  · rename_i one two
...
def stable_close (i j k : ℕ) (h : Nat.dist j k = 1) :
    stable (FreeMonoid.of i) (of j * of k * of j) := by
  intro a b grid_uvab u' v' u_eq v_eq
  rw [BraidMonoidInf.singleton_eq u_eq]
  rcases BraidMonoidInf.triplet_eq_C h v_eq with ⟨one, two, three, four⟩
  · use a, b
    exact ⟨grid_uvab, ⟨⟨by trivial⟩, ⟨by trivial⟩⟩⟩
  rename_i working_h
  rw [working_h.1]
  ...
  rcases trichotomous_dist_C i j
  · rename_i hij
    have ve := helpier_ij h0 _ _ (or_dist_iff.mp hij.1) rfl rfl
    rw [ve.1] at h1
    rcases trichotomous_dist_C i k
    · rename_i hik
      have hik := hik.1
    ...
    · rename_i hik
      ...
      exact H1.trans ((ConGen.Rel.mul (BraidMonoidInf.braid_rel h) (ConGen.Rel.refl _)).trans
        (H3.trans H4))
    rename_i k_is
    rw [k_is.1, Nat.dist_comm, h] at hij
    linarith [hij.1]
  rename_i hij
  rcases hij
  · rename_i hij
    ...
    · rename_i hik
      ...
      rw [hf.1, d₁u₂.1, hi.1, e₁f₂.1, e₂f.1, e₂f.2]
    ...
    rcases hik
    · rename_i hik
      rcases or_dist_iff_eq_C h
      · rename_i jk
        rcases or_dist_iff_eq_C hij
        · rename_i ij
          rw [← jk.1, ← ij.1, Nat.dist_two] at hik
          linarith [hik.1]
        rename_i ij
        rw [jk.1.symm.trans ij.1] at hik
        simp at hik
        exact hik.1.elim
      rename_i jk
      rcases or_dist_iff_eq_C hij
      · rename_i ij
        have H : i = k := by linarith [ij.1, jk.1]
        rw [H] at hik
        simp at hik
        exact hik.1.elim
      rename_i ij
      rw [← jk.1] at ij
      rw [← ij.1, Nat.dist_comm, Nat.dist_two] at hik
      linarith [hik.1]
    rename_i k_is
    have k_is := k_is.1
    rw [← k_is] at h
    ...
    rw [hc.1, c₁u₁.1, c₂g.1, one_mul] at h3
  ...
  rw [ve.2, one_mul, ug.2, af.1, af.2, j_is.1]
  rcases trichotomous_dist_C i k
  · rename_i hik
    rw [j_is.1, h] at hik
    linarith [hik.1]
...
noncomputable def stable_swap : stable u v → stable v u := by
...
def stable_first_one : stable 1 v := by
  ...
  intro nv
  rw [Ha, Hb]
  unfold PresentedMonoid.rel at nv
  sorry
  -- induction nv with
  -- | of x y _ =>
  --   rename_i br
  ...
  --   use 1, o
  --   exact
  --     ⟨grid_top_bottom_word o, ⟨rfl, PresentedMonoid.sound (ConGen.Rel.trans one two)⟩⟩
  -- | mul one two _ _ =>
  --   rename_i n _ p _ _
  --   rw [h]
  --   use 1, n * p
  --   exact ⟨grid_top_bottom_word (n * p), ⟨rfl, PresentedMonoid.sound (ConGen.Rel.mul one two)⟩⟩

def stable_second_one : stable a 1 := by
  ...
  -- · rename_i e f g _
  --   use e, 1
  --   exact ⟨grid_sides_word _, ⟨by rw [← Hc]; exact PresentedMonoid.sound (ConGen.Rel.symm g), rfl⟩⟩
...
def stable_braid_elem {w y : FreeMonoid ℕ} (h : braid_rels_m_inf_C w y) :
    ∀ a, stable (of a) w := by
  rcases h
  · exact fun a ↦ stable_close a _ _ dist_succ
  exact fun a ↦ stable_far_apart a _ _ (or_dist_iff.mpr (Or.inl (by assumption)))

def stable_braid_elem_symm {w y : FreeMonoid ℕ} (h : braid_rels_m_inf_C y w) :
    ∀ a, stable (of a) w := by
  rcases h
  · intro a
    apply stable_close
...
noncomputable def reg_helper (ih : ∀ (u v a b : FreeMonoid ℕ), n ≥ u.length + b.length → grid u v a b →
    ∀ (u' v' : FreeMonoid ℕ), BraidMonoidInf.mk u = BraidMonoidInf.mk u' →
    BraidMonoidInf.mk v = BraidMonoidInf.mk v' → Σ a' b', grid u' v' a' b' ×
    PLift (BraidMonoidInf.mk a = BraidMonoidInf.mk a')
    × PLift (BraidMonoidInf.mk b = BraidMonoidInf.mk b'))
    (br : braid_rels_m_inf f g) (gr : grid e (i * f * j) c d) (len : n + 1 ≥ e.length + d.length) :
    Σ a' b', grid e (i * g * j) a' b' ×
    PLift (BraidMonoidInf.mk c = BraidMonoidInf.mk a') × PLift (BraidMonoidInf.mk d = BraidMonoidInf.mk b') := by
  ...
  -- rcases ih _ _ a₂ d₂ H_len gr_bottom_middle tail mid' rfl top_middle_fact.2.2.1 with
  ...
  --   PresentedMonoid.sound <| ConGen.Rel.mul (PresentedMonoid.exact top_middle_fact.2.1.1)
  --   (PresentedMonoid.exact bottom_middle_fact.2.1.1)
  ...
  --   (PresentedMonoid.exact bottom_middle_fact.2.2.1)) (PresentedMonoid.exact right_fact.2.2.1)

noncomputable def symm_helper (ih : ∀ (u v a b : FreeMonoid ℕ), n ≥ u.length + b.length → grid u v a b →
    ∀ (u' v' : FreeMonoid ℕ), BraidMonoidInf.mk u = BraidMonoidInf.mk u' →
    BraidMonoidInf.mk v = BraidMonoidInf.mk v' → Σ a' b', grid u' v' a' b' ×
    PLift (BraidMonoidInf.mk a = BraidMonoidInf.mk a') × PLift (BraidMonoidInf.mk b = BraidMonoidInf.mk b'))
    (br : braid_rels_m_inf f g) (gr : grid e (i * g * j) c d) (len : n + 1 ≥ e.length + d.length) :
    Σ a' b', grid e (i * f * j) a' b' ×
    PLift (BraidMonoidInf.mk c = BraidMonoidInf.mk a')
    × PLift (BraidMonoidInf.mk d = BraidMonoidInf.mk b') := by
...
def stable_second (ih : ∀ (u v a b : FreeMonoid ℕ), n ≥ u.length + b.length → grid u v a b →
    ∀ (u' v' : FreeMonoid ℕ), BraidMonoidInf.mk u = BraidMonoidInf.mk u' →
    BraidMonoidInf.mk v = BraidMonoidInf.mk v' → Σ a' b', grid u' v' a' b' ×
    PLift (BraidMonoidInf.mk a = BraidMonoidInf.mk a') × PLift (BraidMonoidInf.mk b = BraidMonoidInf.mk b'))
    (b_is : BraidMonoidInf.mk f = BraidMonoidInf.mk i) :
    ∀ (d : FreeMonoid ℕ), n + 1 ≥ a.length + d.length →
    ∀ (c : FreeMonoid ℕ), grid a f c d → Σ a' b', grid a i a' b' ×
    PLift (BraidMonoidInf.mk c = BraidMonoidInf.mk a') × PLift (BraidMonoidInf.mk d = BraidMonoidInf.mk b') := by
  sorry
...
def stability (u v : FreeMonoid ℕ) : stable u v := by
  have H1 : ∀ t u v, ∀ a b, t >= u.length + b.length → grid u v a b → ∀ u' v',
      BraidMonoidInf.mk u = BraidMonoidInf.mk u' →
      BraidMonoidInf.mk v = BraidMonoidInf.mk v' → Σ a' b',
      grid u' v' a' b' × PLift (BraidMonoidInf.mk a = BraidMonoidInf.mk a') ×
      PLift (BraidMonoidInf.mk b = BraidMonoidInf.mk b') := by
    intro t
    induction t with
    | zero =>
