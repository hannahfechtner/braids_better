...
theorem braid_rels_multi_of_separated {i j n : ℕ} {i_n : i < n} {j_n : j < n} (apart : i + 2 ≤ j) :
...
def BraidMonoidFin.toBraidMonoidInf {n : ℕ} : (BraidMonoidFin n) →* BraidMonoidInf :=
  PresentedMonoid.lift _ (BraidMonoidFin.toBraidMonoidInf_helper _)

@[simp]
theorem BraidMonoidFin.toBraidMonoidInf_of {n : ℕ} (i : Fin n.pred) :
    BraidMonoidFin.toBraidMonoidInf (BraidMonoidFin.of _ i) = BraidMonoidInf.of i.1 := PresentedMonoid.lift_of _ _
