...
noncomputable def universalMonoidHom {G₁ : Type} [Group G₁] (f : α → G₁)
  ...
  apply PresentedMonoid.lift_hom f (PresentedMonoid.lift_eq_lift_of_rel f universal_h)
...
theorem universalMonoidHom_unique {G₁ : Type} [Group G₁] (f : α → G₁)
  ...
  | mul_of head tail ih =>
    ...
    erw [(PresentedMonoid.lift_hom f (lift_eq_lift_of_rel f universal_h)).map_mul, ← ih]
...
private theorem lift_numeratorHom_eq_one_of_mem_free_group_set_of_function (r : FreeGroup α)
  ...
  rw [map_mul, map_inv, ← FreeMonoid.lift_eq_FreeGroup_lift_comp_of_apply,
      ← FreeMonoid.lift_eq_FreeGroup_lift_comp_of_apply, lift_of_eq_mk_of_mulHom,
