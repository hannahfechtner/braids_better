...
--this is the same thing as apply_of, I was being silly
theorem pml_to_presented_group_to_mk
    ...
    (PresentedGroup.of a : PresentedGroup (pm_rels_to_pg_rels rels)) := rfl
...
private theorem fraction_identity_works
    ...
    (PresentedMonoid rels) _) = (1 : @OreLocalization _ _ _ (@oreSetSelf' _ rels h1 h)
    (PresentedMonoid rels) _) := by
  intro r r_in
  rcases r_in with ⟨a, b, one, two⟩
  simp only [Set.mem_setOf_eq] at b
  rw [map_mul, map_inv, ← lift_eq_lift_lift_of, ← lift_eq_lift_lift_of]
  have H0 : @OreLocalization.numeratorHom _ _ _ (@oreSetSelf' _ rels h1 h) (PresentedMonoid.mk rels a.1) =
    @OreLocalization.numeratorHom  _ _ _ (@oreSetSelf' _ rels h1 h) (PresentedMonoid.mk rels a.2) := by
    congr 1
    apply PresentedMonoid.sound
    exact PresentedMonoid.rel_alone b
  -- super hacky to make it OreLocalization.instMonoid instead of DivInvMonoid
  have H := @lift_of_eq_mk_of_mulHom α rels _ (@OreLocalization.instMonoid _ _ _ (@oreSetSelf' _ rels h1 h)) a.1 ((@OreLocalization.numeratorHom _ _ ⊤
    (@oreSetSelf' _ rels h1 h)))
  have H2 := @lift_of_eq_mk_of_mulHom α rels _ (@OreLocalization.instMonoid _ _ _ (@oreSetSelf' _ rels h1 h)) a.2 ((@OreLocalization.numeratorHom _ _ ⊤
    (@oreSetSelf' _ rels h1 h)))
  erw [H, H2, H0]
