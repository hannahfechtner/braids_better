...
theorem count_up_pop {b n : ℕ} {h : n>b} : count_up (n-b) b n =
  ...
  have H : ∀ t, ∀ i j, j-i = t → i < j → count_up (j-i) i j = (count_up (j-1-i) i (j-1)) * ↑(j-1) := by
    ...
    · next plus_one_lt =>
      have sub : l-(k+1)=h := by
        have h_is : h = Nat.pred (h + 1) := rfl
        rw [h_is, ←eq_h]
        exact Nat.sub_succ l k
...
theorem count_down_pop {i j : ℕ} { h : i <= j} : count_down ((j+1)-i) (j+1) i =
  ...
  have H : ∀ t, ∀ i j, j-i = t → i <= j → count_down ((j+1)-i) (j+1) i =
    ...
    · next plus_one_lt =>
      specialize hk (k+1) (l)
      have sub : l-(k+1)=h := by
        have h_is : h = Nat.pred (h + 1) := by exact rfl
        rw [h_is, ←eq_h]
        exact Nat.sub_succ l k
      specialize hk sub
...
theorem sigma_neg_big_first {i j : ℕ} (h: i<=j) : sigma_neg (j+1) i = (of j) * sigma_neg j i := by
  ...
  simp only [h1, h2, ge_iff_le, ite_false]
  rw [count_down_pop]
...
theorem sigma_length {i j : ℕ} (h : i<j) : length (sigma_neg i j) = j-i := by
  ...
  rename_i h lt_k ih
  rw [sigma_neg_last]
  · rw [add_tsub_cancel_right, length_mul, ih, length_of]
    exact (Nat.sub_add_comm <| Nat.le_of_succ_le lt_k).symm
