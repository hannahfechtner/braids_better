...
def top_bottom_word (u : FreeMonoid ℕ) : GridData 1 u u 1 := by
  match u with
    | [] => exact GridData.empty
    | head :: tail => exact GridData.horizontal (GridData.top_bottom head) (top_bottom_word tail)

def top_left_word (u : FreeMonoid ℕ) : GridData u u 1 1 := by
  match u with
    | [] => exact GridData.empty
    | head :: tail =>
      apply GridData.horizontal
        (GridData.vertical (GridData.top_left head) (GridData.sides_word tail)) (top_left_word tail)
...
theorem braid_eq (h : GridData a b c d) :
  ...
  apply Grid.braid_monoid_eq_of_grid (to_grid h)
