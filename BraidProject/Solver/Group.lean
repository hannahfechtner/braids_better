...
structure ReverseResult (L : List (ℕ × Bool)) where
  ...
  derivation : SemiThueData reversing L out

open SignedList in
private def reverse_word_helper_from_reverse_pair_nonempty_false_suffix_empty_true_prefix (l1 : ℕ × Bool) (l2 : List (ℕ × Bool))
...
private def reverse_word_helper_from_reverse_pair_nonempty_false_suffix_nonempty_true_prefix (l1 : ℕ × Bool) (l2 : List (ℕ × Bool))
    (sfpc : (separate_first_pair (l1 :: l2)).1 ++
      (separate_first_pair (l1 :: l2)).2.1 ++
      (separate_first_pair (l1 :: l2)).2.2 = l1 :: l2)
    (a1 : ℕ × Bool) (a2 : List (ℕ × Bool))
...
private def reverse_word_helper_from_reverse_pair_nonempty_false_suffix (l1 : ℕ × Bool) (l2 : List (ℕ × Bool))
...
private def reverse_word_helper_from_reverse_pair (l1 : ℕ × Bool) (l2 : List (ℕ × Bool))
    (sfpc : (separate_first_pair (l1 :: l2)).1 ++ (separate_first_pair (l1 :: l2)).2.1 ++ (separate_first_pair (l1 :: l2)).2.2 =
      l1 :: l2)
    (a1 : ℕ × Bool) (a2 : List (ℕ × Bool)) (b1 : ℕ × Bool) (b2 c : List (ℕ × Bool))
    (hs : separate_first_pair (l1 :: l2) = (a1 :: a2, b1 :: b2, c))
...
private def reverse_word_pair_case
...
def reverse_word (L : List (ℕ × Bool)) : ReverseResult L :=
  ...
  | ([], (b, c)) => by
    ...
    rw [← sfpc]
    apply SemiThueData.append_left rc.derivation
  | (a1::a2, ([], c)) => by
