...
def infix_cons_ne_C (h : List.Infix' (a :: b)  (c :: d)) (ne : a ≠ c) : List.Infix' (a :: b) d := by
...
def infix_cons_cons_ne_double_C (h : List.Infix' [a, b] (c1 :: c2 :: d)) (ne : b ≠ c2) : List.Infix' [a, b] (c2 :: d) := by
  rcases h with ⟨w, t, hwt⟩
  cases w with
  | nil =>
...
def infix_cons_cons_def_C (h : List.Infix' [a, b] (c :: d :: e)) : (PLift (a = c ∧ b =d)) ⊕ List.Infix' [a, b] (d :: e):= by
  rcases h with ⟨w, t, hwt⟩
  cases w with
  | nil =>
    simp only [List.nil_append, List.cons_append, List.cons.injEq] at hwt
...
def infix_append_right_C (h : List.Infix'  l1 l2) : List.Infix' l1 (l2 ++ l3) := by
  rcases h with ⟨w, t, hwt⟩
  use w, t ++ l3
  rw [← hwt.1]
  exact {down := by simp}

def infix_append_left_C (h : List.Infix' l1 l2) : List.Infix' l1 (l3 ++ l2) := by
  rcases h with ⟨w, t, hwt⟩
  use l3 ++ w, t
  rw [← hwt.1]
  exact {down := by simp}

theorem infix_length_le_C (h : List.Infix' l1 l2) : l1.length ≤ l2.length := by
  rcases h with ⟨w, t, ⟨hwt⟩⟩
...
@[simp]
theorem infix_not_nil (h : List.Infix' l1 []) (h2 : l1.length > 0) : False := by
  apply infix_length_le_C at h
  simp at h
  rw [h] at h2
  simp at h2

def infix_cons_C {l1 l2 : List α} (h : l1.Infix' l2) : l1.Infix' (a :: l2) := by
  rcases h with ⟨w, t, ⟨hwt⟩⟩
  rw [← hwt]
  use a ::w, t
...
@[simp]
def double_not_infix_nil : List.Infix' [a, b] [] → Empty := by
  intro h
  apply infix_not_nil at h
  simp at h

def List.Infix'.trans (h1 : List.Infix' a b) (h2 : List.Infix' b c) : List.Infix' a c := by
...
def is_true_append (h : is_true (a ++ b)) : is_true a × is_true b := by
  constructor
  · exact fun x hx => h x {down := (List.mem_append_left b hx.1)}
  exact fun x hx => h x {down := (List.mem_append_right a hx.1)}

def is_true_of_true_true (h1 : is_true a) (h2 : is_true b) : is_true (a ++ b) := by
  intro x ⟨h⟩
  simp at h
  exact {down := by
              cases h with
...
def in_order_rest (h : in_order (head :: t)) : in_order t := by
  ...
  | heada :: taila =>
    use taila, a2
    constructor
    · exact fun _ hx => ha.1 _ ⟨(List.mem_cons_of_mem heada hx.1)⟩
    constructor
    · exact ha.2.1
...
def in_order_of_true (h : is_true L) : in_order L := by
  use L, []
  constructor
  · exact h
  constructor
...
def in_order_of_false (h : is_false L) : in_order L := by
  use [], L
  constructor
  · exact is_true_nil
  constructor
...
noncomputable def in_order_append (h : in_order (a++b)) : in_order a × in_order b := by
  ...
  constructor
  · rw [← spec.1] at a1_true
    exact fun _ hx => a1_true _ ⟨(List.mem_append_right _ hx.1)⟩
  exact ⟨a2_false, ⟨spec.right⟩⟩

def in_order_nil {α} : in_order ([] : List (α × Bool)) := by use [], []; simp; exact ⟨
  is_true_nil, ⟨is_false_nil, ⟨trivial⟩ ⟩⟩

def skeleton_order (a : List (α × Bool)) := Σ a1 a2, is_false a1 × is_true a2 × PLift (a = a1 ++ a2)

def skeleton_order_nil {α} : skeleton_order ([] : List (α × Bool)) := by
  use [], []
  simp
  exact ⟨is_false_nil, ⟨is_true_nil, ⟨trivial⟩⟩⟩
...
def is_true_split (h : is_true (a :: b)) : is_true [a] × is_true b := by
  change is_true ([a]++b) at h
  exact is_true_append h

def is_false_split (h : is_false (a :: b)) : is_false [a] × is_false b := by
