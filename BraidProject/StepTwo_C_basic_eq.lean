...
noncomputable def grid_style_to_cell (h : grid_style i j) : Σ a b c d,
    CellData (option_to_list a) (option_to_list b) c d × PLift (i = [(a, false), (b, true)] ∧ j = to_horizontal_edge c ++ to_vertical_edge d) := by
  cases h with
  | basic n =>
    use some n, some n, [], []
  ...
  | empty a b ha ha1 hb hb1 =>
    ...
    | nil =>
      ...
      | nil =>
        ...
        use b, m, u
        exact ⟨h3, ⟨h4, ⟨List.SuffixData.nil, List.PrefixData.nil⟩⟩⟩
      | cons head tail =>
        rw [← l_is]
    ...
    | cons head tail =>
      ...
      | cons headb tailb =>
        ...
        have := skeleton_cons_cons hg (is_false_of_append ha1).1 (is_true_of_append hb1).2 (by assumption)
        rcases this with ⟨b, m, u, h3, h4⟩
        use b, m, u
        exact ⟨h3, ⟨h4, ⟨List.SuffixData.nil, List.PrefixData.nil⟩⟩⟩
...
noncomputable def step_two (ha : is_false a) (ha1 : a.length > 0)
  ...
  | step h1 h2 ih =>
    rcases ih hab (SemiThueDerivation.toSemiThue h1) with ⟨bot, mid, up, h3, ⟨h4⟩⟩
