...
theorem prod_eq_of {a b : FreeMonoid α} {i : α} (h : a * b = of i) :
    (a = 1 ∧ b = of i) ∨ (a = of i ∧ b = 1) := by
  have H : length (a * b) = 1 := by
    rw [h]
    exact length_of _
  rw [length_mul] at H
...
theorem lift_eq_FreeGroup_lift_comp_of_apply {G₁ : Type} [Group G₁] (f : α → G₁) (a : FreeMonoid α) :
    lift f a = (FreeGroup.lift f) (lift FreeGroup.of a) := by
  simpa using congrArg (fun φ : FreeMonoid α →* G₁ => φ a)
    (lift_eq_FreeGroup_lift_comp_of (f := f))
...
def congr_iso {α : Type u_1} {β : Type u_2} (e : α ≃ β) : FreeMonoid α ≃* FreeMonoid β := by
  apply MulEquiv.mk' ⟨map e.toFun, map e.invFun, _, _⟩
  · simp
  all_goals
  intro x
  simp
...
theorem neq_one {c : FreeMonoid α} (h : c ≠ 1) : ∃ a b, c = of a * b := by
  induction c using FreeMonoid.inductionOn'
  · exact (h rfl).elim
  rename_i head tail _
  use head, tail

def mapNatToFin (n : ℕ) (a : FreeMonoid ℕ) (bound : ∀ x ∈ a, x<n) : FreeMonoid (Fin n) :=
  (pmap Fin.mk a) bound

@[simp]
theorem mapNatToFin_empty {n h} : mapNatToFin n 1 h = 1 := rfl

theorem mapNatToFin_singleton (a : ℕ) (b : FreeMonoid ℕ)
    (bounded_a : ∀ x, x ∈  of a → x < n) (bounded_b : ∀ x, x ∈ b → x < n)
    (bounded_ab : ∀ x ∈ of a * b, x < n) : mapNatToFin n (of a * b) bounded_ab =
    mapNatToFin n (of a) bounded_a * mapNatToFin n b bounded_b := by
  rfl

theorem mapNatToFin_mul (a b : FreeMonoid ℕ) (bounded_a : ∀ x∈ a, x < n) (bounded_b : ∀ x∈ b, x < n) (bounded_ab : ∀ x∈ a * b, x < n) :
    mapNatToFin n (a * b) bounded_ab = mapNatToFin n a bounded_a  *  mapNatToFin n b bounded_b := by
  induction a using FreeMonoid.inductionOn'
  · grind [one_mul, mapNatToFin_empty]
  rename_i ha ta ihta
  have bounded_ha : ∀ x, x ∈ of ha → x < n :=
    fun t h => bounded_a t (mem_mul.mpr (Or.inl h))
  have bounded_ta : ∀ x, x ∈ ta → x < n :=
    fun t h => bounded_a t (mem_mul.mpr (Or.inr h))
  have bounded_tab : ∀ x, x ∈ ta * b → x < n := fun t h => bounded_ab t
      ((congrArg (fun _a => t ∈ _a) (mul_assoc (of ha) ta b)).mpr
      ((congrArg id (propext mem_mul)).mpr (Or.inr h)))
  rw [mapNatToFin_singleton ha ta bounded_ha bounded_ta]
  conv => rhs; rw [mul_assoc]
  rw [← ihta bounded_ta bounded_tab]
  rfl

theorem lt_of_mem_map_val {n : ℕ} (u : FreeMonoid (Fin n)) (x : ℕ)
    (h : x ∈ map (fun i => ↑i) u) : x < n := by
  rcases mem_map.mp h with ⟨a, ha⟩
  rw [← ha.2]
  exact a.isLt

theorem mapNatToFin_map_val (u : FreeMonoid (Fin n)) :
    mapNatToFin n (map (fun i => i.val) u) (lt_of_mem_map_val u) = u := by
  induction u using FreeMonoid.inductionOn'
  · simp only [map_one]
    rw [mapNatToFin_empty]
  rename_i h t iht
  have : map (fun i => i.val) (of h * t) = of h.val * map (fun i => i.val) (t) := rfl
  simp only [this]
  rw [mapNatToFin_singleton h _ (by grind [mem_of]) (by grind), iht]
  rfl

theorem mapNatToFin_map_val_mul_right : mapNatToFin n (map (fun i => i.val) u  *  v') bounded_a =
    u * mapNatToFin n v' v'_bound := by
  conv =>
  {
    enter [1]
    apply mapNatToFin_mul (map (fun i => i.val) u) v' (lt_of_mem_map_val u) v'_bound
  }
