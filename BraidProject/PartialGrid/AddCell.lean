...
noncomputable def grid_style.toPartialGrid (h : grid_style i j) (ha : a.length > 0) (hb : b.length > 0)
...
noncomputable def grid_style.toPartialGrid_extend_top_side (h2 : grid_style i j)
...
noncomputable def grid_style.toPartialGrid_extend_left_side (h2 : grid_style i j)
    (a_is : a = head :: tail ++ a2)
...
noncomputable def grid_style.toPartialGrid_extend_both_sides (gs : grid_style i j)
...
noncomputable def PartialGrid.add_any_cell_with_length (h : PartialGrid a b bot mid up)
  ...
  | empty a b ha ha1 hb hb1 =>
    ...
    | nil =>
      ...
      | nil =>
        ...
        have H := grid_style.toPartialGrid hg (by assumption) hb (by assumption) i_is
    ...
    | cons head tail =>
      ...
      | cons headb tailb =>
        ...
        rw [← k_is, ← l_is]
        have := grid_style.toPartialGrid_extend_both_sides hg (is_false_of_append ha1).1 (is_true_of_append hb1).2 (by assumption)
...
--         rcases grid_style_trivial.toPartialGrid hg ha hb i_is ab_is with ⟨nb, nm, nu, h3, h4, hl⟩
--         refine ⟨nb, nm, nu, h3, ?_, List.SuffixData.nil, List.PrefixData.nil, ?_⟩
--         · rw [List.nil_append, List.append_nil]; exact h4
--         constructor; simp [PartialGrid.length, ← hl.1]
--       | cons head tail =>
...
--         rcases grid_style_trivial.toPartialGrid_extend_top_side hg fe_new b_split ha1 ha hb1 ab_is i_is
--           with ⟨nb, nm, nu, h3, h4, hl⟩
--         refine ⟨nb, nm, nu, h3, h4, List.SuffixData.nil, List.PrefixData.nil, ?_⟩
--         constructor; simp [PartialGrid.length, ← hl.1]
--     | cons head tail =>
--       cases l with
--       | nil =>
...
--         rcases grid_style_trivial.toPartialGrid_extend_left_side hg a_eq ha1 hb1 ab_is i_is b_eq' hb
--           with ⟨nb, nm, nu, h3, h4, hl⟩
--         refine ⟨nb, nm, nu, h3, h4, List.SuffixData.nil, List.PrefixData.nil, ?_⟩
--         constructor; simp [PartialGrid.length, ← hl.1]
--       | cons headb tailb =>
...
--         subst a_eq b_eq
--         rcases grid_style_trivial.toPartialGrid_extend_both_sides hg ht_false hb_true i_is
--           with ⟨nb, nm, nu, h3, h4, hl⟩
--         refine ⟨nb, nm, nu, h3, h4, List.SuffixData.nil, List.PrefixData.nil, ?_⟩
--         constructor; simp [PartialGrid.length, ← hl.1]
--   | horizontal_append_one g1 g2 ih1 ih2 =>
...
noncomputable def step_two (ha : is_false a) (ha1 : a.length > 0)
    (hb : is_true b) (hb1 : b.length > 0) (hs : SemiThueData grid_style (a ++ b) c) :
    (Σ bot mid up, (h : PartialGrid a b bot mid up) × PLift (bot ++ mid ++ up = c) ×
    PLift (h.length = SemiThueData.grid_style.length hs)) := by
  generalize hab : a ++ b = ab at hs
  have hss := SemiThueData.grid_style.toSemiThueDataDerivation_with_length hs
  rw [hss.2]
  induction (hss).1 with
  | refl =>
    have := hab.symm
    subst this
    ...
    rfl
  | step h1 h2 ih =>
    rcases ih hab (SemiThueDataDerivation.grid_style.toSemiThueData_with_length h1).1
      (by use h1; rw [(SemiThueDataDerivation.grid_style.toSemiThueData_with_length h1).2.1]) with
      ⟨bot, mid, up, h3, ⟨h4⟩, ⟨hl⟩⟩
    rcases PartialGrid.add_any_cell_with_length h3 h2 h4 with ⟨b, m, u, h3, h4, hl'⟩
    use b, m, u, h3
    constructor
    · exact h4
    constructor
