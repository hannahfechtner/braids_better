...
def gridl_of_grid (h : grid a b c d) : Nonempty (gridl a b c d) := by
  induction h with
  | empty =>
    apply Nonempty.intro gridl.empty
  | top_bottom i => apply Nonempty.intro (gridl.top_bottom i)
  | sides i => apply Nonempty.intro (gridl.sides i)
  | top_left i => apply Nonempty.intro (gridl.top_left i)
  | adjacent i k h => apply Nonempty.intro (gridl.adjacent i k h)
  | separated i j h => apply Nonempty.intro (gridl.separated i j h)
...
theorem grid_of_gridl (h : gridl a b c d) : grid a b c d := by
  induction h with
...
def unicity_c {a b c d c' d'} (h1 : gridl a b c d) : gridl a1 b1 c' d' → a = a1 → b = b1 → PLift (c' = c) × PLift (d' = d) := by
  intro h2 a_is b_is
  rw [← a_is, ← b_is] at h2
  have H := unicity (grid_of_gridl h1) _ _ (grid_of_gridl h2)
  exact ⟨⟨H.1⟩, ⟨H.2⟩⟩

noncomputable def existence : ∀ a b, ∃ c d, Nonempty (gridl a b c d) := by
  intro a b
...
noncomputable def existence_s : ∀ a b, Σ c d, gridl a b c d :=
