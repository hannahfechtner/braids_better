import BraidProject.PartialGrids_S
...
theorem right_frontier_is_false (h : PartialGrid a b c d e n) : is_false e := by
  induction h with
  | single_zero => exact is_false_up
  | single_one  => exact is_false_up
  | empty => simp
...
theorem top_frontier_is_true (h : PartialGrid a b c d e n) : is_true b := by
  induction h with
  | single_zero => exact is_true_over
  | single_one  => exact is_true_over
  | empty  => assumption
...
theorem left_frontier_is_false (h : PartialGrid a b c d e n) : is_false a := by
  induction h with
    | single_zero => exact is_false_up
    | single_one => exact is_false_up
    | empty => assumption
...
theorem bottom_frontier_is_true (h : PartialGrid a b c d e n) : is_true c := by
  induction h with
    | single_zero => exact is_true_over
    | single_one => exact is_true_over
    | empty => simp
...
theorem left_length_pos (h : PartialGrid a b c d e n) : a.length > 0 := by
  induction h with
  | single_zero  => exact to_up_len_pos
  | single_one  => exact to_up_len_pos
  | empty => assumption
  | horizontal_append_one => assumption
  | horizontal_append => assumption
  | vertical_append_one =>
...
theorem top_length_pos (h : PartialGrid a b c d e n) : b.length > 0 := by
  induction h with
  | single_zero => exact to_over_len_pos
  | single_one => exact to_over_len_pos
  | empty => assumption
...
theorem mid_length_neq_one (h : PartialGrid a b c d e n) : d.length ≠ 1 := by
  intro hd
  induction h with
  | single_zero => simp at hd
  | single_one => simp at hd
...
theorem grid_rel_means (h : grid_style i j) : ∃ a b c d, i = [(a, false), (b, true)] ∧
    (cell_zero (option_to_cell a) (option_to_cell b) c d ∨ cell_one (option_to_cell a) (option_to_cell b) c d) ∧ j = to_over d ++ to_up c := by
  cases h with
  | basic n =>
    use some n, some n, [], []
    exact ⟨rfl, ⟨.inr (cell_one.top_left n), rfl⟩⟩
  | over n =>
    use some n, none, [n], []
    exact ⟨rfl, ⟨.inl (cell_zero.sides n), rfl⟩⟩
  | up n =>
    use none, some n, [], [n]
    exact ⟨rfl, ⟨.inl (cell_zero.top_bottom n), rfl⟩⟩
  | empty =>
    use none, none, [], []
    exact ⟨rfl, ⟨.inl cell_zero.empty, rfl⟩⟩
  | apart h =>
    rename_i i j
    use some i, some j, [i], [j]
    exact ⟨rfl, ⟨.inr (cell_one.separated i j (or_dist_iff.mp h)), rfl⟩⟩
  | close h =>
    rename_i i j
    use some i, some j, [i, j], [j, i]
    exact ⟨rfl, ⟨.inr (cell_one.adjacent i j h), rfl⟩⟩

theorem skeleton_one_one (h : grid_style i j) (ha : a.length > 0) (hb : b.length > 0)
    (i_is : i = [(a3, false), (b3, true)]) (ab : [(a3, false), (b3, true)] = a ++ b) :
    ∃ bot mid up n, PartialGrid a b bot mid up n ∧ bot ++ mid ++ up = j := by
  ...
  rw [happ.1, happ.2]
  rcases h_cell with h1 | h2
  · rw [← over_oc, ← up_oc]
    constructor
    · constructor
      · exact PartialGrid.single_zero h1
      rw [List.append_nil]
      exact j_is.symm
  constructor
  · constructor
    · rw [← over_oc, ← up_oc]
      exact PartialGrid.single_one h2
    rw [List.append_nil]
    exact j_is.symm
...
theorem skeleton_one_cons (h2 : grid_style i j) (fe : a ++ b = ([(a3, false), (b3, true)] ++ head :: tail))
    ...
    ∃ bot mid up n, PartialGrid a b bot mid up n ∧ bot ++ mid ++ up = [] ++ j ++ head :: tail := by
  have ht_true : is_true (head :: tail) := by
    rw [b_is] at hb
    exact (is_true_append hb).2
  rcases grid_rel_means h2 with ⟨a2, b2, c2, d2, i_is', hc1 | hc2, j_is⟩
  · use to_over d2, to_up c2 ++ head :: tail, [], 0
    constructor
    · have H2 := PartialGrid.empty (to_up c2) (head :: tail) (by simp [to_up_len_pos]) is_false_up (by simp) ht_true
      have H3 := PartialGrid.horizontal_append_one (PartialGrid.single_zero hc1) H2
    ...
    rw [j_is]
    simp
  use to_over d2, to_up c2 ++ head :: tail, [], 1
  constructor
  · have H2 := PartialGrid.empty (to_up c2) (head :: tail) (by simp [to_up_len_pos]) is_false_up (by simp) ht_true
    have H3 := PartialGrid.horizontal_append_one (PartialGrid.single_one hc2) H2
    simp only [up_oc, over_oc, List.singleton_append, List.append_nil] at H3
    have helper := i_is.symm.trans i_is'
    simp only [List.cons.injEq, Prod.mk.injEq, and_true] at helper
    have ha : a = [(a2, false)] := by
...
theorem skeleton_cons_one (h2 : grid_style i j) (a_is : a = head :: tail ++ a2)
    ...
    ∃ bot mid up n, PartialGrid a b bot mid up n ∧ bot ++ mid ++ up = head :: tail ++ j ++ [] := by
  ...
  rcases h_cell with hc1 | hc2
  · have H3 := PartialGrid.vertical_append_one (PartialGrid.single_zero hc1) H2
    use [], head::tail ++ to_over d2, to_up c2, 0
    constructor
    · rw [a_is]
    ...
    rw [j_is]
    simp
  have H3 := PartialGrid.vertical_append_one (PartialGrid.single_one hc2) H2
  use [], head::tail ++ to_over d2, to_up c2, 1
  constructor
  · rw [a_is]
    have H := i_is.symm.trans i_is'
    simp at H
...
theorem skeleton_cons_cons (gs : grid_style i j) (ha : is_false (head :: tail)) (hb : is_true (headb :: tailb))
    ...
    ∃ bot mid up n, PartialGrid (head :: tail ++ [(a3, false)]) ([(b3, true)] ++ headb :: tailb) bot mid up n ∧
...
def PartialGrid.extend_bottom (h : PartialGrid a b c d e n) (a2) (h2 : is_false a2) (h3 : a2 ≠ []) :
    PartialGrid (a2 ++ a) b [] (a2 ++ c ++ d) e n := by
  induction h with
  | single_zero h =>
    cases a2 with
    | nil => simp at h3
    | cons head tail =>
      rename_i d
      have H := PartialGrid.vertical_append_one (PartialGrid.single_zero h)
  ...
  | single_one h =>
    ...
    | cons head tail =>
      rename_i d
      have H := PartialGrid.vertical_append_one (PartialGrid.single_one h)
        (PartialGrid.empty (head :: tail) (to_over d) (by simp) h2 to_over_len_pos is_true_over)
      rw [List.nil_append] at H
      rw [List.append_nil]
      exact H
...
def PartialGrid.extend_side (h : PartialGrid a b c d e n) (b2) (h2 : is_true b2) (h3 : b2 ≠ []) :
    PartialGrid a (b ++ b2) c (d ++ e ++ b2) [] n := by
  induction h with
  | single_zero h =>
    cases b2 with
    | nil => simp at h3
    | cons head tail =>
      rename_i c _
      have H := PartialGrid.horizontal_append_one (PartialGrid.single_zero h)
  ...
  | single_one h =>
    ...
    | cons head tail =>
      rename_i c _
      have H := PartialGrid.horizontal_append_one (PartialGrid.single_one h)
          (PartialGrid.empty (to_up c) (head :: tail) to_up_len_pos is_false_up (by simp) h2)
      rw [List.append_nil] at H
      rw [List.nil_append]
      exact H
...
theorem middle_frontier_nil_or_caps (h : PartialGrid a b c d e n) : middle_spec d := by
  induction h with
  | single_zero h =>
    left; rfl
  | single_one h =>
  ...
  | cons head tail ih =>
    ...
    | cons headk tailk =>
      ...
      have h2 : is_true tail ∨ is_false tail := by
        ...
        · exact Or.inl (is_true_split h1).2
        exact Or.inr (is_false_split h2).2
...
theorem double_split_helper' {bot2 mid2 bot3 mid3 up3 k l : List (Option ℕ × Bool)} {a1 b1 : Option ℕ}
  ...
  | cons head tail ih =>
    ...
    | cons headl taill =>
      ...
      specialize @ih taill (is_true_split hbot2).2 h.2
      rcases ih with ⟨m1, m2, hm12, k_is⟩ | ⟨m3, m4, hm34, l_is⟩
      · left
        use m1, m2
        constructor
...
theorem double_split_horiz' {bot2 mid2 bot3 mid3 up3 k l : List (Option ℕ × Bool)} {a1 b1 : Option ℕ}
  ...
  · left
    ...
    have : ∃ ender, hk::tk = h3 :: t3 ++ ender := by
      ...
      rcases prefix_false (is_false_split h4).2 k12_is.2.2 with ⟨f, spec⟩
      rw [← spec]
      use f
...
def add_cell (h : PartialGrid a b bot mid up n) (hg : grid_style i j) (fe : bot ++ mid ++ up = k ++ i ++ l) :
    ∃ nb nm nu n2, PartialGrid a b nb nm nu n2 ∧ nb ++ nm ++ nu = k ++ j ++ l ∧ up <:+ nu ∧ bot <+: nb := by
    ...
    | nil =>
      ...
      | cons head tail =>
        ...
        have := skeleton_one_cons hg fe b_is ha1 ha hb1 (by rw [← a_is] at i_is; exact i_is)
          (by assumption)
        rcases this with ⟨b, m, u, n, h3, h4⟩
        use b, m, u, n
        exact ⟨h3, ⟨h4, ⟨List.nil_suffix, List.nil_prefix⟩⟩⟩
    | cons head tail =>
      cases b2 with
      ...
      | cons headb tailb =>
        ...
        exact ⟨h3, ⟨h4, ⟨List.nil_suffix, List.nil_prefix⟩⟩⟩
  | horizontal_append_one g1 g2 ih1 ih2 =>
    rename_i n1 n2 a2 b2 bot2 up2 b3 bot3 mid3 up3
    rcases big_split_first (bottom_frontier_is_true g1) fe with ⟨k₁, k₂, k_is, eq_rest, k₁_is⟩
    rcases @ih2 k₂ l eq_rest with ⟨bot1, mid1, up1, n, pg1, fe1, h5, h6⟩
    use bot2 ++ bot1, mid1, up1, n1 + n
    constructor
  ...
  | horizontal_append h g1 g2 g1_ih g2_ih =>
    rename_i n1 n2 a2 b2 bot2 mid2 up2 b3 bot3 mid3 up3
    ...
    · rcases hl with ⟨k₁, k₂, k_is, k1_is, k2_is⟩
      rcases g2_ih k2_is.symm with ⟨bot3, mid3, up3, n3, hpg, hf⟩
      use bot2, mid2 ++ bot3++mid3, up3, n1 + n3
      constructor
    ...
    have H3 : bot2 ++ mid2 ++ up2 = k ++ [(a1, false), (b1, true)] ++ (l₁ ++ up2) := by
      rw [← l2_is]
      simp
    rcases @g1_ih k (l₁ ++ up2) H3 with ⟨bot4, mid4, up4, n4, hpg, hf, ⟨to_add, spec⟩, h6⟩
    cases mid4 with
    | nil =>
      cases to_add with
      | nil =>
        use bot4 ++ bot3, mid3, up3, n4 + n2
      ...
      | cons heade taile =>
        use bot4, (heade :: taile) ++ bot3 ++ mid3, up3, n4 + n2
        constructor
    ...
    | cons head tail =>
      cases to_add with
      | nil =>
        use bot4, head::tail ++ bot3 ++ mid3, up3, n4 + n2
        rw [List.nil_append] at spec
        rw [← spec] at hpg
        constructor
        ...
        exact ⟨List.suffix_refl up3, h6⟩
      | cons heade taile =>
        use bot4, head::tail ++ (heade :: taile) ++ bot3 ++ mid3, up3, n4 + n2
        constructor
        · have lf : is_false (heade :: taile) := by
        ...
        exact ⟨List.suffix_refl up3, h6⟩
  | vertical_append_one g1 g2 ih1 ih2 =>
      rename_i a2 b2 bot2 up2 n1 b3 bot3 mid3 up3 n2
      rcases big_split (right_frontier_is_false g1) fe with ⟨l₁, l₂, l_is, eq_rest, l₂_is⟩
      rcases @ih2 _ _ eq_rest with ⟨bot1, mid1, up1, n, pg1, fe1, h5, h6⟩
      use bot1, mid1, up1 ++ up2, n1 + n
      constructor
  ...
  | vertical_append g1 g2 h g1_ih g2_ih =>
    rename_i a b bot mid up n1 a2 bot2 mid2 up2 n2
    ...
    · specialize @g1_ih (bot ++ k2) l (by rw [List.append_assoc, ← k2_is]; simp)
      rcases g1_ih with ⟨nb, nm, nu, n3, pg, fe', upp, botp⟩
      ...
      | nil =>
        ...
        | cons head tail =>
          use bot2, mid2 ++ up2 ++ head :: tail, nu, n3 + n2
          constructor
      ...
      | cons head tail =>
        ...
        | nil =>
          ...
          exact ⟨upp, List.prefix_rfl⟩
        | cons head1 tail1 =>
          use bot2, mid2 ++ up2 ++ head :: tail ++ head1 :: tail1, nu, n3 + n2
          constructor
          · have H1 : is_true (head:: tail) := by
              have H : is_true nb := bottom_frontier_is_true pg
              rw [← spec] at H
          ...
          exact ⟨upp, List.prefix_rfl⟩
    rw [← l2_is] at g2_ih
    rcases @g2_ih k l1 (by simp) with ⟨nb, nm, nu, n3, pg, fe', upp, botp⟩
    use nb, nm ++ nu ++mid, up, n1 + n3
    constructor
...
theorem step_two (ha : is_false a) (ha1 : a.length > 0) (hb : is_true b) (hb1 : b.length > 0) :
    SemiThue grid_style (a ++ b) c → (∃ bot mid up n, PartialGrid a b bot mid up n ∧ bot ++ mid ++ up = c) := by
  intro h
  ...
  | one_step h1 h2 ih =>
    rcases ih ell (one_step_equiv_reg.mpr h1) with ⟨bot, mid, up, n, h3, h4⟩
    rcases add_cell h3 h2 h4 with ⟨b, m, u, n2, h3, h4⟩
    use b, m, u, n2
    exact ⟨h3, h4.1⟩
...
def get_n (h : SemiThue reversing (a1 ++ a2) c)
  (ha1 : is_false a1) (a1_len : a1.length >0) (ha2 : is_true a2) (a2_len : a2.length > 0) : False := by
  have H := rev_to_grid h
  rcases H with ⟨c', spec1, spec2, spec3⟩
  ...
  rcases step_two (is_false_to_option ha1) a1_len (is_true_to_option ha2) a2_len spec1 with ⟨bot, mid, up, n1, pg, c_is⟩
