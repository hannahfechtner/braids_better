...
theorem all_ones_length_pg (h : pgf a b c d e) : a = [(none, false)] → b = [(none, true)] → h.length = 0 := by
  induction h with
  | single_gridt h =>
    cases h with
    | empty => simp [pgf.length]
    | top_bottom i => simp [pgf.length]
    | sides i => simp [pgf.length]
    | top_left i =>
      intro h1
      simp [to_up] at h1
    | adjacent i k h =>
    ...
    | separated i j h =>
      intro h1
      simp [to_up] at h1
  | empty a b ha ha1 hb hb => simp [pgf.length]
  | horizontal_append_one g1 g2 g1_ih g2_ih =>
    intro a_is b_is
    rcases List.append_eq_singleton_iff.1 b_is with hb | hb
    · have H := pgf.top_length_pos g1
      rw [hb.1] at H
      simp at H
    have H := pgf.top_length_pos g2
    rw [hb.2] at H
    simp at H
  ...
  | vertical_append_one g1 g2 g1_ih g2_ih =>
    ...
    · have H := pgf.left_length_pos g2
      rw [hb.1] at H
      simp at H
    have H := pgf.left_length_pos g1
    rw [hb.2] at H
    simp at H
...
theorem top_bottom_length_pg (h : pgf a b c d e) : a = [(none, false)] → b = [(some i, true)] → h.length = 0 := by
  induction h with
  | single_gridt h =>
    cases h with
    | empty => simp [pgf.length]
...
theorem side_side_length_pg {a b c d e i} (h : pgf a b c d e) : a = [(some i, false)] → b = [(none, true)] → h.length = 0 := by
  induction h with
  | single_gridt h =>
    cases h with
    | empty => simp [pgf.length]
...
theorem top_left_length_pg {a b c d e i} (h : pgf a b c d e) : a = [(some i, false)] → b = [(some i, true)] →
  ...
  | single_gridt h =>
    ...
    | top_bottom i => simp
    | sides i => simp
    | top_left i => simp [pgf.length]
    | adjacent i k h =>simp [pgf.length]
    | separated i j h => simp [pgf.length]
...
theorem adjacent_length_pg (h : pgf a b c d e) : a = [(some i, false)] → b = [(some j, true)] →
...
theorem separated_length_pg (h : pgf a b c d e) : a = [(some i, false)] → b = [(some k, true)] →
...
def split_vertically_pg' (h : pgf a b c d e)  := ∀ b₁ b₂, b = b₁ ++ b₂ →
  b₁.length > 0 → b₂.length > 0 →
  (Σ mid c1 d1 c2 d2,
  (h1 : pgf a b₁ c1 d1 mid) × (h2 : pgf mid b₂ c2 d2 e) ×
  PLift (c ++ d = c1 ++ d1 ++ c2 ++ d2) ×
  PLift (h.length = h1.length + h2.length)) ⊕
  (Σ d1 d2, (h1 : pgf a b₁ c d1 []) × PLift (h.length = h1.length) ×
    PLift (e = []) × PLift (d = d1 ++ d2) × PLift (b₂ = d2))

-- def split_vertically_pg_1 (h : pgf a b c d e)  := ∀ b₁ b₂, b = b₁ ++ b₂ →
--   b₁.length > 0 → b₂.length > 0 →
--   (Σ mid c1 d1 c2 d2,
--   (h1 : pgf a b₁ c1 d1 mid) × (h2 : pgf mid b₂ c2 d2 e) ×
--   PLift (c ++ d = c1 ++ d1 ++ c2 ++ d2) ×
--   PLift (h.length = h1.length + h2.length) ×
--     (∀ {c3 m3 c4 d4}, pgf a b₁ c3 [] m3 → pgf m3 b₂ c4 d4 e →
...
theorem not_both_empty : pgf a b c d e → d = [] → e = [] → False := by
  intro h
  induction h with
  | single_gridt h =>
    intro ha hb
...
theorem not_both_empty_early : pgf a b c d e → c = [] → d = [] → False := by
  intro h
  induction h with
  | single_gridt h =>
    intro ha hb
...
theorem pg_not_mid_right_empty : pgf a b c [] [] → False := fun h => not_both_empty h rfl rfl

noncomputable def pgf.extend_bottom_w_len (h : pgf a b c d e) (a2) (h2 : is_false a2) (h3 : a2 ≠ []) :
    (h1 : pgf (a2 ++ a) b [] (a2 ++ c ++ d) e) × PLift (h.length = h1.length):= by
  induction h with
  | single_gridt h =>
    cases a2 with
    | nil => simp at h3
    | cons head tail =>
      rename_i d
      rw [List.append_nil]
      have H := pgf.vertical_append_one (pgf.single_gridt h)
        (pgf.empty (head :: tail) (to_over d) (by simp) h2 to_over_len_pos is_true_over)
      use pgf.vertical_append_one (pgf.single_gridt h)
        (pgf.empty (head :: tail) (to_over d) (by simp) h2 to_over_len_pos is_true_over)
      constructor
      simp [pgf.length]
  | empty a b ha ha1 hb hb =>
    rw [List.append_nil, ← List.append_assoc]
    use pgf.empty (a2 ++ a) b (by rw [List.length_append]; omega) (is_false_of_false_false h2 ha1) (by assumption) hb
    simp [pgf.length]
    exact ⟨trivial⟩
  | horizontal_append_one g1 g2 ih1 ih2 =>
    ...
    rw [← H]
    use pgf.horizontal_append (by simp; exact Or.inl (List.length_pos_iff.mpr h3)) ih1.1 g2
    simp [pgf.length]
    exact ih1.2
  ...
  | vertical_append_one g1 g2 ih1 ih2 =>
    ...
    use pgf.vertical_append_one g1 ih2.1
    simp [pgf.length]
    exact ih2.2
  | vertical_append g1 g2 h ih1 ih2 =>
    rw [← List.append_assoc, ← List.append_assoc, ← List.append_assoc]
    use pgf.vertical_append g1 ih2.1 h
    simp [pgf.length]
    exact ih2.2

noncomputable def splittable_vertically_of_pg' (h : pgf a b c d e) : split_vertically_pg' h := by
  induction h with
  | single_gridt h =>
    cases h with
    | empty =>
  ...
  | empty a b ha ha1 hb hb1 =>
    ...
    use b₂
    use pgf.empty a b₁ ha ha1 b₁_len itb₁
    constructor
    · exact ⟨by simp [pgf.length]⟩
    constructor
    · exact ⟨rfl⟩
    constructor
    · constructor
  ...
  | horizontal_append_one g1 g2 g1_ih g2_ih =>
    ...
    · left
      ...
      | [] =>
        use c2 ++ bot2, mid2
        use h1
        use pgf.horizontal_append_one h2 g2
        rw [List.append_nil, List.append_nil] at long
        constructor
        · rw [long]
          exact ⟨by simp⟩
        exact ⟨by simp [pgf.length, h_len, ← add_assoc]⟩
      | d21 :: d22 =>
        use c2, d21 :: d22 ++ bot2 ++ mid2
        use h1
        use pgf.horizontal_append (by simp) h2 g2
        repeat rw [List.append_nil] at long
        simp [long, h_len, pgf.length, ← add_assoc]
        exact ⟨⟨trivial⟩, ⟨trivial⟩⟩
    right
    rcases bad with ⟨d1, d2, h3, h_len, end_is⟩
    have H := pgf.left_length_pos g2
  ...
  | horizontal_append h g1 g2 g1_ih g2_ih =>
    ...
    · rcases List.cases_C from_middle with ⟨⟨silly⟩⟩ | ⟨⟨fm_l⟩⟩
      ...
      · left
        ...
        use pgf.horizontal_append h g1 h1
        use h2
        constructor
        · constructor
      ...
      rw [one.1]
      use (mid1 ++ bot2 ++ d1), d2
      use pgf.horizontal_append h g1 h3
      constructor
      · exact ⟨by rw [pgf.length, h_len.1, pgf.length]⟩
      constructor
      · exact end_is.1
    ...
    · left
      ...
      | d21 :: d22 =>
        ...
        use h1
        use pgf.horizontal_append (by simp) h2 g2
        simp [← List.append_assoc, long, h_len, pgf.length, ← add_assoc]
      ...
      | [] =>
        ...
        | [] =>
          ...
          · use mid2 ++ mid, c3, d3, c4, d4
            use pgf.vertical_append_one h1 i1
            use pgf.vertical_append_one h2 i2
            constructor
            · exact long1
            constructor
            simp [pgf.length, len1.1, len]
            omega
          rcases bad with ⟨d1, d2, h3, len1⟩
          match up2 with
          | [] =>
            use mid, bot2, d1, c2, []
            use pgf.vertical_append_one h1 h3
            use h2
            constructor
            · constructor
              rw [List.append_assoc, List.append_assoc]
  ...
  | vertical_append g1 g2 h g1_ih g2_ih =>
    ...
    · match d1 with
      | [] =>
        ...
        | [] =>
          ...
          subst bot1_is
          use pgf.vertical_append_one h1 g2
          match up2 with
          | [] =>
            use h2
            constructor
            ...
            exact ⟨by omega⟩
          | up21 :: up22 =>
            use (pgf.extend_bottom_w_len h2 (up21 :: up22) (pgf.right_frontier_is_false g2) (by simp)).1
            constructor
            · constructor
              simp [mid_is]
            constructor
            simp [pgf.length, len,
              (pgf.extend_bottom_w_len h2 (up21 :: up22) (pgf.right_frontier_is_false g2) (by simp)).2.1]
            omega
        | c21 :: c22 =>
          left
          ...
          · use mid3 ++ mid, c3, d3, c4
            ...
            | d21 :: d22 =>
              use d4 ++ up2 ++ d21 :: d22
              use pgf.vertical_append_one h1 i1
              use pgf.vertical_append h2 i2 (by simp)
              constructor
              · constructor
      ...
      | d11 :: d12 =>
        ...
        | c11 :: c12 =>
          ...
          subst bot1_is
          use pgf.vertical_append h1 g2 (by simp)
          use h2
          constructor
          · constructor
            simp [long]
    ...
    have H : d1.length > 0 := by
      ...
      | d11 :: d12 => simp
    use pgf.vertical_append h3 g2 H
    constructor
    · simp [pgf.length, len]
      exact ⟨trivial⟩
    constructor
...
noncomputable def split_horizontally_pg (h : pgf a b c d e) := ∀ a1 a2,
  a = a2 ++ a1 → a1.length > 0 → a2.length > 0 → (Σ mid d1 e1 d2 e2,
  (h1 : pgf a1 b mid d2 e2) × (h2 : pgf a2 mid c d1 e1) ×
  PLift (d1 ++ e1 ++ d2 ++e2 = d ++ e) × PLift (h.length = h1.length + h2.length)) ⊕
  (Σ db c1 drest, (h1 : pgf a1 b c1 drest e) × PLift (d = db ++ c1 ++ drest) ×
...
noncomputable def reflect (h : pgf a b c d e) :
    (h1 : pgf (bool_swap b) (bool_swap a) (bool_swap e) (bool_swap d) (bool_swap c)) ×
  ...
  | single_gridt h =>
    ...
    cases h with
    | empty =>
      use pgf.single_gridt (cell.empty)
      exact ⟨rfl⟩
    | top_bottom i =>
      use pgf.single_gridt (cell.sides i)
      exact ⟨rfl⟩
    | sides i =>
      use pgf.single_gridt (cell.top_bottom i)
      exact ⟨rfl⟩
    | top_left i =>
      use pgf.single_gridt (cell.top_left i)
      exact ⟨rfl⟩
    | adjacent i k h =>
      use pgf.single_gridt (cell.adjacent k i (by rw [Nat.dist_comm] at h; exact h))
      exact ⟨rfl⟩
    | separated i j h =>
      use pgf.single_gridt (cell.separated j i (by rw [Or.comm] at h; exact h))
      exact ⟨rfl⟩
  | empty a b ha ha1 hb hb1 =>
    rw [bool_swap_append]
    rw [← bool_swap_length] at ha
    rw [← bool_swap_length] at hb
    use pgf.empty (bool_swap b) (bool_swap a) hb (bool_swap_true hb1) ha (bool_swap_false ha1)
    simp [pgf.length]
    exact ⟨trivial⟩
  | horizontal_append_one g1 g2 g1_ih g2_ih =>
    ...
    use pgf.vertical_append_one h3 h4
    exact ⟨by simp [pgf.length, len3.1, len4.1]⟩
  | horizontal_append h g1 g2 g1_ih g2_ih =>
    rw [bool_swap_append, bool_swap_append, bool_swap_append, ← List.append_assoc]
    rcases g1_ih with ⟨h3, len3⟩
    rcases g2_ih with ⟨h4, len4⟩
    rw [← bool_swap_length] at h
    use pgf.vertical_append h3 h4 h
    exact ⟨by simp [pgf.length, len3.1, len4.1]⟩
  | vertical_append_one g1 g2 g1_ih g2_ih =>
    rw [bool_swap_append, bool_swap_append]
    rcases g1_ih with ⟨h3, len3⟩
    rcases g2_ih with ⟨h4, len4⟩
    use pgf.horizontal_append_one h3 h4
    exact ⟨by simp [pgf.length, len3.1, len4.1]⟩
  | vertical_append g1 g2 h g1_ih g2_ih =>
    rw [bool_swap_append, bool_swap_append, bool_swap_append, ← List.append_assoc]
    rcases g1_ih with ⟨h3, len3⟩
    rcases g2_ih with ⟨h4, len4⟩
    rw [← bool_swap_length] at h
    use pgf.horizontal_append h h3 h4
    exact ⟨by simp [pgf.length, len3.1, len4.1]⟩

noncomputable def reflect_one_two (h : pgf a1 b1 c d e) : a1 = bool_swap a → b1 = bool_swap b →
  (h1 : pgf b a (bool_swap e) (bool_swap d) (bool_swap c)) × PLift (h.length = h1.length) := by
  intro a_eq b_eq
...
noncomputable def reflect_two_five (h : pgf a b1 c d e1) : b1 = bool_swap b → e1 = bool_swap e →
  (h1 : pgf b (bool_swap a) e (bool_swap d) (bool_swap c)) × PLift (h.length = h1.length) := by
  intro b_eq e_eq
  apply congr_arg bool_swap at b_eq
  rw [bool_swap_idem] at b_eq
  rw [← b_eq]
...
noncomputable def reflect_one_two_three (c e) (h : pgf a1 b1 c1 d e) :
    a1 = bool_swap a → b1 = bool_swap b → c1 = bool_swap c →
    (h1 : pgf b a (bool_swap e) (bool_swap d) c) × PLift (h.length = h1.length) := by
...
noncomputable def reflect_all (a b c d e) (h : pgf a1 b1 c1 d1 e1) :
    a1 = bool_swap a → b1 = bool_swap b → c1 = bool_swap c → d1 = bool_swap d → e1 = bool_swap e →
    (h1 : pgf b a e d c) × PLift (h.length = h1.length) := by
...
noncomputable def splittable_horizontally_of_pg (h : pgf a b c d e) :
...
noncomputable def pgf.extend_side_w_len  (h : pgf a b c d e) (b2) (h2 : is_true b2) (h3 : b2 ≠ []) :
    (h1 : pgf a (b ++ b2) c (d ++ e ++ b2) []) × PLift  (h.length = h1.length) := by
  rcases reflect h with ⟨h4, ⟨len⟩⟩
  have ⟨h5, ⟨len2⟩⟩ := pgf.extend_bottom_w_len h4 (bool_swap b2) (bool_swap_true h2)
...
theorem pg_empty {a b c d e} (h : pgf a b c d e)
...
theorem pg_top_bottom {a b c d e} (h : pgf a b c d e)
...
theorem pg_side_side {a b c d e} (h : pgf a b c d e)
...
theorem pg_top_left {a b c d e} (h : pgf a b c d e)
...
theorem pg_adjacent {a b c d e} (h : pgf a b c d e)
...
theorem pg_separated {a b c d e} (h : pgf a b c d e)
  (ha : a = [(some i, false)]) (hb : b = [(some j, true)]) (hd : d = []) (hij : i.dist j > 1):
