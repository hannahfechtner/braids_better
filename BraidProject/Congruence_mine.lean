...
congruence relation containing a binary relation is also given (see `Con'Gen`).
...
structure Con' [Mul M] extends Setoid M where
  /-- Con'gruence relations are closed under multiplication -/
...
/-- The inductively defined smallest additive congruence relation containing a given binary
    relation. -/
inductive AddCon'Gen.Rel [Add M] (r : M → M → Prop) : M → M → Prop
  | of : ∀ x y, r x y → AddCon'Gen.Rel r x y
  | refl : ∀ x, AddCon'Gen.Rel r x x
  | symm : ∀ {x y}, AddCon'Gen.Rel r x y → AddCon'Gen.Rel r y x
  | trans : ∀ {x y z}, AddCon'Gen.Rel r x y → AddCon'Gen.Rel r y z → AddCon'Gen.Rel r x z
  | add : ∀ {w x y z}, AddCon'Gen.Rel r w x → AddCon'Gen.Rel r y z → AddCon'Gen.Rel r (w + y) (x + z)
#align add_con_gen.rel AddCon'Gen.Rel

/-- The inductively defined smallest multiplicative congruence relation containing a given binary
    relation. -/
@[to_additive AddCon'Gen.Rel]
inductive Con'Gen.Rel [Mul M] (r : M → M → Prop) : M → M → Prop
  | of : ∀ x y, r x y → Con'Gen.Rel r x y
  | refl : ∀ x, Con'Gen.Rel r x x
  | symm : ∀ {x y}, Con'Gen.Rel r x y → Con'Gen.Rel r y x
  | trans : ∀ {x y z}, Con'Gen.Rel r x y → Con'Gen.Rel r y z → Con'Gen.Rel r x z
  | mul : ∀ {w x y z}, Con'Gen.Rel r w x → Con'Gen.Rel r y z → Con'Gen.Rel r (w * y) (x * z)
#align con_gen.rel Con'Gen.Rel

/-- The inductively defined smallest multiplicative congruence relation containing a given binary
    relation. -/
@[to_additive addCon'Gen "The inductively defined smallest additive congruence relation containing
a given binary relation."]
def con'Gen [Mul M] (r : M → M → Prop) : Con' M :=
  ⟨⟨Con'Gen.Rel r, ⟨Con'Gen.Rel.refl, Con'Gen.Rel.symm, Con'Gen.Rel.trans⟩⟩, Con'Gen.Rel.mul⟩
#align con_gen con'Gen
#align add_con_gen addCon'Gen
...
/-- Con'gruence relations are reflexive. -/
...
/-- Con'gruence relations are symmetric. -/
...
/-- Con'gruence relations are transitive. -/
...
#align con.con_gen_eq Con'.con'Gen_eq
#align add_con.add_con_gen_eq AddCon'.addCon'Gen_eq

/-- The smallest congruence relation containing a binary relation `r` is contained in any
    congruence relation containing `r`. -/
@[to_additive addCon'Gen_le "The smallest additive congruence relation containing a binary
relation `r` is contained in any additive congruence relation containing `r`."]
theorem con'Gen_le {r : M → M → Prop} {c : Con' M} (h : ∀ x y, r x y → c x y) :
    con'Gen r ≤ c := by rw [con'Gen_eq]; exact sInf_le h
#align con.con_gen_le Con'.con'Gen_le
#align add_con.add_con_gen_le AddCon'.addCon'Gen_le

/-- Given binary relations `r, s` with `r` contained in `s`, the smallest congruence relation
    containing `s` contains the smallest congruence relation containing `r`. -/
@[to_additive addCon'Gen_mono "Given binary relations `r, s` with `r` contained in `s`, the
smallest additive congruence relation containing `s` contains the smallest additive congruence
relation containing `r`."]
theorem con'Gen_mono {r s : M → M → Prop} (h : ∀ x y, r x y → s x y) : con'Gen r ≤ con'Gen s :=
  con'Gen_le fun x y hr => Con'Gen.Rel.of _ _ <| h x y hr
#align con.con_gen_mono Con'.con'Gen_mono
#align add_con.add_con_gen_mono AddCon'.addCon'Gen_mono

/-- Con'gruence relations equal the smallest congruence relation in which they are contained. -/
@[to_additive (attr := simp) addCon'Gen_of_addCon' "Additive congruence relations equal the smallest
additive congruence relation in which they are contained."]
theorem con'Gen_of_con (c : Con' M) : con'Gen c = c :=
  le_antisymm (by rw [con'Gen_eq]; exact sInf_le fun _ _ => id) Con'Gen.Rel.of
#align con.con_gen_of_con Con'.con'Gen_of_con
#align add_con.add_con_gen_of_con AddCon'.addCon'Gen_of_addCon'
#align add_con.add_con_gen_of_add_con AddCon'.addCon'Gen_of_addCon'

-- Porting note: removing simp, simp can prove it
/-- The map sending a binary relation to the smallest congruence relation in which it is
    contained is idempotent. -/
@[to_additive addCon'Gen_idem "The map sending a binary relation to the smallest additive
congruence relation in which it is contained is idempotent."]
theorem con'Gen_idem (r : M → M → Prop) : con'Gen (con'Gen r) = con'Gen r :=
  con'Gen_of_con _
#align con.con_gen_idem Con'.con'Gen_idem
#align add_con.add_con_gen_idem AddCon'.addCon'Gen_idem

/-- The supremum of congruence relations `c, d` equals the smallest congruence relation containing
    the binary relation '`x` is related to `y` by `c` or `d`'. -/
@[to_additive sup_eq_addCon'Gen "The supremum of additive congruence relations `c, d` equals the
smallest additive congruence relation containing the binary relation '`x` is related to `y`
by `c` or `d`'."]
theorem sup_eq_con'Gen (c d : Con' M) : c ⊔ d = con'Gen fun x y => c x y ∨ d x y := by
  rw [con'Gen_eq]
  apply congr_arg sInf
  simp only [le_def, or_imp, ← forall_and]
#align con.sup_eq_con_gen Con'.sup_eq_con'Gen
#align add_con.sup_eq_add_con_gen AddCon'.sup_eq_addCon'Gen
...
theorem sup_def {c d : Con' M} : c ⊔ d = con'Gen (⇑c ⊔ ⇑d) := by rw [sup_eq_con'Gen]; rfl
...
/-- The supremum of a set of congruence relations `S` equals the smallest congruence relation
    ...
    `c`'. -/
@[to_additive sSup_eq_addCon'Gen "The supremum of a set of additive congruence relations `S` equals
...
theorem sSup_eq_con'Gen (S : Set (Con' M)) :
    sSup S = con'Gen fun x y => ∃ c : Con' M, c ∈ S ∧ c x y := by
  rw [con'Gen_eq]
  apply congr_arg sInf
  ext
  exact ⟨fun h _ _ ⟨r, hr⟩ => h hr.1 hr.2, fun h r hS _ _ hr => h _ _ ⟨r, hS, hr⟩⟩
#align con.Sup_eq_con_gen Con'.sSup_eq_con'Gen
#align add_con.Sup_eq_add_con_gen AddCon'.sSup_eq_addCon'Gen
...
theorem sSup_def {S : Set (Con' M)} :
    sSup S = con'Gen (sSup ((⇑) '' S)) := by
  rw [sSup_eq_con'Gen, sSup_image]
  congr with (x y)
...
protected def gi : @GaloisInsertion (M → M → Prop) (Con' M) _ _ con'Gen DFunLike.coe where
  choice r _ := con'Gen r
  gc _ c := ⟨fun H _ _ h => H <| Con'Gen.Rel.of _ _ h, @fun H => con'Gen_of_con c ▸ con'Gen_mono H⟩
  le_l_u x := (con'Gen_of_con x).symm ▸ le_refl x
  choice_eq _ _ := rfl
#align con.gi Con'.gi
...
def mapGen (f : M → N) : Con' N :=
  con'Gen fun x y => ∃ a b, f a = x ∧ f b = y ∧ c a b
...
theorem mapOfSurjective_eq_mapGen {c : Con' M} {f : M → N} (H : ∀ x y, f (x * y) = f x * f y)
  ...
  rw [← con'Gen_of_con (c.mapOfSurjective f H h hf)]; rfl
