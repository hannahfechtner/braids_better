...
inductive SemiThue_one_step (rels : List α → List α → Type) : List α → List α → Type
...
noncomputable def one_step_equiv_reg {a b : List α} : (SemiThue rels a b → SemiThue_one_step rels a b) ×
    (SemiThue_one_step rels a b → SemiThue rels a b) := by
  constructor
  · intro h
    induction h
...
noncomputable def SemiThue_cons (h : SemiThue rels a b) : SemiThue rels (c :: a) (c :: b) := by
  induction h with
...
noncomputable def SemiThue_caboose (h : SemiThue rels a b) : SemiThue rels (a ++ [c]) (b ++ [c]) := by
  induction h with
