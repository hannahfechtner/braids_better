...
theorem common_right_mul (u v : BraidMonoidInf) : ∃ v' u', u * v' = v * u' := by
...
theorem common_right_mul_mk (u v) : ∃ v' u', BraidMonoidInf.mk (u*v') = ⟦v*u'⟧ := by
  rcases common_right_mul ⟦u⟧ ⟦v⟧ with ⟨u', v', huv⟩
  induction u' with | h u''
  induction v' with | h v''
  use u'', v''
  exact huv

theorem common_left_mul (u v : BraidMonoidInf) : ∃ u' v', u' * u = v' * v := by
  rcases common_right_mul (BraidMonoidInf.reverse_braid u)
...
instance : IsCommonLeftMultipleMul (BraidMonoidInf) where
  common_left_multiple := common_left_mul

instance : IsCommonRightMultipleMul (BraidMonoidInf) where
  common_right_multiple := common_right_mul
