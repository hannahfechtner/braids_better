...
theorem all_ones_pg (h : PartialGrid a b c d e) : a = [] → b = [] → c = [] ∧ d = [] ∧ e = [] ∧ h.length = 0 := by
  induction h with
  | single_gridt h =>
    cases h; all_goals simp [to_up_plain, PartialGrid.length]
  | empty a b ha ha1 hb hb => aesop
  | horizontal_append_one g1 g2 g1_ih g2_ih => rw [PartialGrid.length]; aesop
  | horizontal_append h g1 g2 g1_ih g2_ih => rw [PartialGrid.length]; aesop
  | vertical_append_one g1 g2 g1_ih g2_ih => rw [PartialGrid.length]; aesop
  | vertical_append g1 g2 h g1_ih g2_ih => rw [PartialGrid.length]; aesop

theorem all_ones_length_pg (h : PartialGrid a b c d e) : a = [] → b = [] → h.length = 0 := by
  exact fun ha hb => (all_ones_pg h ha hb).2.2.2

theorem top_bottom_pg (h : PartialGrid a b c d e) : a = [] → b = [(i, true)] → e = [] ∧ c ++ d = [(i, true)] ∧ h.length = 0 := by
  induction h with
  | single_gridt h =>
    cases h; all_goals simp [to_up_plain, PartialGrid.length]
  | empty a b ha ha1 hb hb => aesop
  | horizontal_append_one g1 g2 g1_ih g2_ih =>
    ...
    · have H := all_ones_pg g1 a_is hb.1
      have H2 := all_ones_length_pg g1 a_is hb.1
      aesop
    specialize g1_ih a_is hb.1
    have H := all_ones_pg g2 g1_ih.1 hb.2
    aesop
  ...
  | vertical_append_one g1 g2 g1_ih g2_ih =>
    ...
    simp at a_is
    simp_all
  | vertical_append g1 g2 ml g1_ih g2_ih =>
    intro a_is b_is
    rw [PartialGrid.length]
    simp at a_is
    simp_all
    rename_i bot _ _ _ _ _ _
    have bot_nil : bot = [] := by
      refine List.eq_nil_iff_length_eq_zero.mpr ?_
      have H := g1_ih.2.1
      apply congr_arg List.length at H
      simp at H
      linarith
    have H := all_ones_pg g2 a_is.1 bot_nil
    aesop

theorem top_bottom_length_pg (h : PartialGrid a b c d e) : a = [] → b = [(i, true)] → h.length = 0 := by
  exact fun ha hb => (top_bottom_pg h ha hb).2.2

theorem side_side_pg (h : PartialGrid a b c d e) (ha : a = [(i, false)]) (hb : b = []) :
  c = [] ∧ d ++ e = [(i, false)] ∧ h.length = 0:= by
  induction h with
  | single_gridt h =>
    cases h; all_goals simp_all [to_over_plain, to_up_plain, ha, hb, PartialGrid.length]
  | empty a b ha ha1 hb hb => aesop
  | horizontal_append_one g1 g2 g1_ih g2_ih => rw [PartialGrid.length]; aesop
  | horizontal_append h g1 g2 g1_ih g2_ih =>
    simp at hb
    ...
    have up_nil : up = [] := by
      refine List.eq_nil_iff_length_eq_zero.mpr ?_
      have H := (g1_ih ha hb.1).2.1
      apply congr_arg List.length at H
      simp at H
      linarith
    have H2 := all_ones_pg g2 up_nil hb.2
    rw [PartialGrid.length]
    aesop
  | vertical_append_one g1 g2 g1_ih g2_ih =>
    rw [PartialGrid.length]
    rcases List.append_eq_singleton_iff.1 ha with ha | ha
    · have H := all_ones_pg g2 ha.1
      aesop
    have H := all_ones_pg g1 ha.2 hb
    aesop
...
theorem side_side_length_pg {a b c d e i} (h : PartialGrid a b c d e) : a = [(i, false)] → b = [] → h.length = 0 := by
  exact fun ha hb => (side_side_pg h ha hb).2.2

-- d is either nil or the both of them; c and e are always empty
theorem top_left_pg {a b c d e i} (h : PartialGrid a b c d e) : a = [(i, false)] → b = [(i, true)] →
  c = [] ∧ e = [] ∧ (d = [] ∨ d = [(i, false), (i, true)]) ∧  (d = [] → h.length = 1) ∧
  (d = [(i, false), (i, true)] → h.length = 0):= by
  induction h with
  | single_gridt h =>
    cases h ; all_goals simp [PartialGrid.length, to_up_plain, to_over_plain]
    · intro h' h''
      rename_i hd
      rw [h', h''] at hd
      simp at hd
  ...
  | horizontal_append_one g1 g2 g1_ih g2_ih =>
    ...
    · have H := side_side_pg g1 a_is hb.1
      ...
      aesop
    specialize g1_ih a_is hb.1
    have H := all_ones_pg g2 g1_ih.2.1 hb.2
    rw [PartialGrid.length]
    aesop
  | horizontal_append h g1 g2 g1_ih g2_ih =>
    ...
    · have H := side_side_pg g1 a_is hb.1
      have H : up = [] := by
        ...
        have H' := congr_arg List.length H.2.1
        rw [List.length_singleton, List.length_append] at H'
        omega
      have H := top_bottom_pg g2 H hb.2
      rw [PartialGrid.length]
      aesop
    specialize g1_ih a_is hb.1
    have H := all_ones_pg g2 g1_ih.2.1 hb.2
    ...
    aesop
  | vertical_append_one g1 g2 g1_ih g2_ih =>
    rename_i a b bot up a1 bot2 mid2 up2
    intro a_is b_is
    rcases List.append_eq_singleton_iff.1 a_is with ha | ha
    · specialize g1_ih ha.2 b_is
      have H := all_ones_pg g2 ha.1 g1_ih.1
      rw [PartialGrid.length]
      aesop
    have H := top_bottom_pg g1 ha.2 b_is
    rw [PartialGrid.length]
    aesop
  | vertical_append g1 g2 h g1_ih g2_ih =>
    rename_i a b bot mid up a1 bot2 mid2 up2
    intro a_is b_is
    rcases List.append_eq_singleton_iff.1 a_is with ha | ha
    · specialize g1_ih ha.2 b_is
      have H := all_ones_pg g2 ha.1 g1_ih.1
    ...
    have bot_is : bot = [] := by
      ...
      rw [List.length_singleton, List.length_append] at H'
      omega
    have H := side_side_pg g2 ha.1 bot_is
    rw [PartialGrid.length]
    aesop

theorem top_left_length_pg {a b c d e i} (h : PartialGrid a b c d e) : a = [(i, false)] → b = [(i, true)] →
  c ++ d ++ e = [] → h.length = 1 := by
  intro ha hb rm
...
noncomputable def PartialGrid.extend_bottom_w_len (h : PartialGrid a b c d e) (a2) (h2 : is_false a2) (h3 : a2 ≠ []) :
  ...
  | single_gridt h =>
    ...
    | cons head tail =>
      ...
      match d with
      | [] =>
        use PartialGrid.single_gridt h

        sorry
      | d1 :: d2 =>
      have H := PartialGrid.vertical_append_one (PartialGrid.single_gridt h)
        (PartialGrid.empty (head :: tail) (to_over_plain (d1 :: d2)) (by simp) h2 (by sorry) (by sorry))
      use PartialGrid.vertical_append_one (PartialGrid.single_gridt h)
        (PartialGrid.empty (head :: tail) (to_over_plain (d1 :: d2)) (by simp) h2 sorry sorry)
      constructor
      ...
      | [] =>
        ...
        | [] =>
          ...
          have hc1 : c1.length > 0 := by
            match c1 with
            | [] =>
              sorry --exact (not_both_empty_early h1 rfl rfl).elim
            | co :: ct => simp
          have hc2 : c2.length > 0 := by
             match c2 with
            | [] =>
              sorry --exact (not_both_empty_early h2 rfl rfl).elim
            | co :: ct => simp
...
theorem bool_swap_to_over_plain : bool_swap (to_over_plain a) = to_up_plain a := by
  induction a with
  | nil => simp [to_over_plain, to_up_plain, bool_swap]
  | cons head tail ih =>
    simp [bool_swap, to_over_plain, ih, to_up_plain]

theorem bool_swap_to_up_plain : bool_swap (to_up_plain a) = to_over_plain a := by
  induction a with
  | nil => simp [to_over_plain, to_up_plain, bool_swap]
  | cons head tail ih =>
    simp [bool_swap, to_up_plain, ih, to_over_plain]
...
noncomputable def splittable_horizontally_of_pg (h : PartialGrid a b c d e) :
  ...
  subst c_nil
  have H0 := reflect_one_two_three e ([] : List (ℕ × Bool)) h3 rfl rfl rfl
  use H0.1
  constructor
  · constructor
