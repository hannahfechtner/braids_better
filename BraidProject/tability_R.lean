import BraidProject.GridsTwo_R
open FreeMonoid

def stable (u v : List (Option ℕ)) := ∀ a b, gridr u v a b → ∀ u' v',
  BraidMonoidInf.mk (to_fm u) = BraidMonoidInf.mk (to_fm u') →
  BraidMonoidInf.mk (to_fm v) = BraidMonoidInf.mk (to_fm v') → ∃ a' b',
    gridr u' v' a' b' ∧ BraidMonoidInf.mk (to_fm a) = BraidMonoidInf.mk (to_fm a') ∧
    BraidMonoidInf.mk (to_fm b) = BraidMonoidInf.mk (to_fm b')

theorem stable_far_apart (i j k : ℕ) (h : Nat.dist j k >= 2) :
    stable [some i] [some j, some k] := by
  intro a b gridr_uvab u' v' u_eq v_eq
  simp [to_fm] at u_eq
  have H := BraidMonoidInf.singleton_eq u_eq
  have H2 := BraidMonoidInf.pair_eq h v_eq

  rcases BraidMonoidInf.pair_eq h v_eq
  · use a, b
    sorry
  rename_i working_h
  rw [working_h]
  rcases splittable_vertically_of_gridr gridr_uvab (of j) (of k) rfl with ⟨u, d, f, h1, h2, rfl⟩
  ...
  · rcases trichotomous_dist i k
    · rename_i hij hik
      ...
      rw [ud.2, af.1, af.2]
      use of i, of k * of j
      exact ⟨gridr.horizontal (gridr.separated i k hik)
        (gridr.separated i j hij),
        ⟨rfl, PresentedMonoid.sound (BraidMonoidInf.comm_rel h)⟩⟩
      -- exact ⟨gridr.horizontal (gridr.separated i k (or_dist_iff.mp hik))
      --   (gridr.separated i j (or_dist_iff.mp hij)),
      --   ⟨rfl, PresentedMonoid.sound (BraidMonoidInf.comm_rel h)⟩⟩
    rename_i last_two
    rcases last_two
    · rename_i one two
      ...
      · rw [Nat.dist_comm] at h
        have H := (gridr.vertical (gridr.separated i j one)
          (gridr.separated k j h))
        -- have H := (gridr.vertical (gridr.separated i j (or_dist_iff.mp one))
        --   (gridr.separated k j (or_dist_iff.mp h)))
        exact gridr.horizontal (gridr.adjacent i k two) H
    ...
    use 1, of j
    exact ⟨gridr.horizontal (gridr.top_left k) (gridr.top_bottom j), ⟨rfl, by rw [mul_one]⟩⟩
  rename_i last_two
  rcases last_two
  · rcases trichotomous_dist i k
    · rename_i one two
      ...
      constructor
      · apply gridr.horizontal (gridr.separated i k two) (gridr.adjacent i j one)
        --apply gridr.horizontal (gridr.separated i k (or_dist_iff.mp two)) (gridr.adjacent i j one)
      have ud := helpier_close h1 i j one rfl rfl
      rw [ud.1] at h2
      rw [ud.2]
      have H_split := splittable_horizontally_of_gridr h2 (of i) (of j) rfl
      ...
      have H1 : PresentedMonoid.rel braid_rels_m_inf (of j * of i * of k) (of j * of k * of i) := by
        rw [mul_assoc, mul_assoc]
        apply ConGen.Rel.mul (ConGen.Rel.refl _) (BraidMonoidInf.comm_rel two)
      exact H1.trans <| ConGen.Rel.mul (BraidMonoidInf.comm_rel h) (ConGen.Rel.refl _)
    rename_i last_two
    rcases last_two
    · rename_i hij hik
      ...
      rw [c₁u₁.2] at hb
      have second_split := splittable_vertically_of_gridr hb (of k) (of i) rfl
      ...
      constructor
      · rw [Nat.dist_comm] at hij
        apply gridr.horizontal (gridr.adjacent i k hik) (gridr.vertical (gridr.adjacent i j hij)
          (gridr.horizontal (gridr.separated k j (by rw [Nat.dist_comm] at h; exact h))
          (gridr.adjacent k i (by rw [Nat.dist_comm] at hik; exact hik))))
        -- apply gridr.horizontal (gridr.adjacent i k hik) (gridr.vertical (gridr.adjacent i j hij)
        --   (gridr.horizontal (gridr.separated k j
      ...
      have H1 : PresentedMonoid.rel braid_rels_m_inf (of j * of i * (of k * (of i * of j)))
        ...
        exact ConGen.Rel.mul (BraidMonoidInf.braid_rel hik) (ConGen.Rel.refl _)
      have H2 : PresentedMonoid.rel braid_rels_m_inf ((of j * of k) * of i * (of k * of j))
        ...
        exact ConGen.Rel.mul (BraidMonoidInf.comm_rel h) (ConGen.Rel.mul (ConGen.Rel.refl _)
          (ConGen.Rel.symm (BraidMonoidInf.comm_rel h)))
      have H3 : PresentedMonoid.rel braid_rels_m_inf (of k * (of j * of i * of j) * of k)
        ...
        apply ConGen.Rel.mul (ConGen.Rel.mul (ConGen.Rel.refl _) (BraidMonoidInf.braid_rel hij))
          (ConGen.Rel.refl _)
  ...
  · rename_i one two
    ...
    rw [one]
    constructor
    · apply gridr.horizontal _ (gridr.top_left j)
      apply gridr.separated
      rw [← one]
      exact two
...
theorem stable_close (i j k : ℕ) (h : Nat.dist j k = 1) :
  ...
  rename_i working_h
  rw [working_h]
  rcases splittable_vertically_of_gridr gridr_uvab (of j * of k) (of j) rfl with
    ⟨u, d, f, h2, h3, rfl⟩
  rcases splittable_vertically_of_gridr h2 (of j) (of k) rfl with ⟨v, e, g, h0, h1, rfl⟩
  rcases trichotomous_dist i j
  · rename_i hij
    ...
    · rename_i hik
      ...
      constructor
      · apply gridr.horizontal (gridr.separated i k hik)
          (gridr.horizontal (gridr.separated i j hij)
          (gridr.separated i k hik))
      -- · apply gridr.horizontal (gridr.separated i k (or_dist_iff.mp hik))
      --     (gridr.horizontal (gridr.separated i j (or_dist_iff.mp hij))
      --     (gridr.separated i k (or_dist_iff.mp hik)))
      constructor
      · rw [af.1]
      rw [ve.2, ug.2, af.2]
      apply PresentedMonoid.sound
    ...
    · rename_i hik
      ...
      rw [ug.1] at h3
      have H_split := splittable_horizontally_of_gridr h3 (of i) (of k) rfl
      ...
      constructor
      · apply gridr.horizontal (gridr.adjacent i k hik) (gridr.horizontal
        (gridr.vertical (gridr.separated i j _) (gridr.adjacent k j _))
        (gridr.vertical (gridr.adjacent i k hik)
        (gridr.horizontal (gridr.vertical (gridr.top_left k) (gridr.sides j))
        (gridr.vertical (gridr.top_bottom i) (gridr.separated j i _)))))
        · exact hij
        · rw [Nat.dist_comm] at h
          exact h
        rw [Nat.dist_comm] at hij
      ...
      --   (gridr.vertical (gridr.adjacent i k hik)
      --   (gridr.horizontal (gridr.vertical (gridr.top_left k) (gridr.sides j))
      --   (gridr.vertical (gridr.top_bottom i) (gridr.separated j i (or_dist_iff.mp _))))))
      ...
      have H1 : PresentedMonoid.rel braid_rels_m_inf (of j * (of k * of i) * (of j * of k))
        ...
        exact ConGen.Rel.mul (ConGen.Rel.mul (ConGen.Rel.refl _) (BraidMonoidInf.comm_rel hij))
          (ConGen.Rel.refl _)
      have H3 : PresentedMonoid.rel braid_rels_m_inf ((of k * of j * of k) * (of i * of k))
        ...
        apply ConGen.Rel.mul (ConGen.Rel.refl _)
        rw [← mul_assoc]
        exact ConGen.Rel.symm (BraidMonoidInf.braid_rel hik)
      have H4 : PresentedMonoid.rel braid_rels_m_inf (of k * of j * (of i * of k * of i))
        ...
        exact ConGen.Rel.mul (ConGen.Rel.mul (ConGen.Rel.mul (ConGen.Rel.refl _)
          (ConGen.Rel.symm (BraidMonoidInf.comm_rel hij))) (ConGen.Rel.refl _)) (ConGen.Rel.refl _)
      exact H1.trans ((ConGen.Rel.mul (BraidMonoidInf.braid_rel h) (ConGen.Rel.refl _)).trans
        (H3.trans H4))
    rename_i k_is
  ...
  · rename_i hij
    ...
    rw [ve.1] at h1
    have H_split := splittable_horizontally_of_gridr h1 (of i) (of j) rfl
    rcases H_split with ⟨u₁, c₁, c₂, ha, hb, hc⟩
    rcases trichotomous_dist i k
    · rename_i hik
      ...
      have H_split := splittable_horizontally_of_gridr h3 _ _ rfl
      rcases H_split with ⟨u₂, d₁, d₂, hd, he, hf⟩
      have d₁u₂ := helpier_close hd _ _ hij rfl rfl
      rw [d₁u₂.2] at he
      have H_split := splittable_horizontally_of_gridr he _ _ rfl
      rcases H_split with ⟨u₃, e₁, e₂, hg, hh, hi⟩
      have H_split := splittable_vertically_of_gridr hg _ _ rfl
      ...
      · apply gridr.horizontal (gridr.separated i k hik)
          (gridr.horizontal (gridr.adjacent i j hij)
          (gridr.vertical (gridr.separated i k hik) (gridr.adjacent j k h)))
      -- · apply gridr.horizontal (gridr.separated i k (or_dist_iff.mp hik))
      --     (gridr.horizontal (gridr.adjacent i j hij)
      --     (gridr.vertical (gridr.separated i k (or_dist_iff.mp hik)) (gridr.adjacent j k h)))
      constructor
      ...
      have H1 : PresentedMonoid.rel braid_rels_m_inf (of j * of i * (of k * of j) * of i)
        ...
        rw [← mul_assoc, ← mul_assoc (of j * of k), ← mul_assoc (of j * of k), mul_assoc _ (of i),
          mul_assoc _ (of k)]
        exact ConGen.Rel.mul (ConGen.Rel.mul (ConGen.Rel.mul (ConGen.Rel.refl _)
          (BraidMonoidInf.comm_rel hik)) (ConGen.Rel.refl _)) (ConGen.Rel.refl _)
      ...
      have H4 : PresentedMonoid.rel braid_rels_m_inf (of k * of j * of k * of i * of j)
        ...
        exact ConGen.Rel.mul (ConGen.Rel.mul (ConGen.Rel.refl _)
          (ConGen.Rel.symm (BraidMonoidInf.comm_rel hik))) (ConGen.Rel.refl _)
    ...
    use 1, of j * of i
    constructor
    · apply gridr.horizontal (gridr.top_left i)
        (gridr.horizontal (gridr.top_bottom j) (gridr.top_bottom i))
    exact ⟨rfl, rfl⟩
  rename_i j_is
  rw [← j_is] at h0
  ...
  · use 1, of k * of j
    constructor
    · apply gridr.horizontal (gridr.adjacent j k h) (gridr.horizontal
        (gridr.vertical (gridr.top_left j) (gridr.sides k))
        (gridr.vertical (gridr.top_bottom k) (gridr.top_left k)))
    exact ⟨rfl, rfl⟩
  rename_i k_is
  rw [j_is.symm.trans k_is]
  use 1, of k * of k
  constructor
  · apply gridr.horizontal (gridr.top_left k)
      (gridr.horizontal (gridr.top_bottom k) (gridr.top_bottom k))
  exact ⟨rfl, rfl⟩
...
theorem stable_swap : stable u v → stable v u := by
  ...
  rcases h d c (gridr_swap gr) u' v' u_is v_is with ⟨a1, b1, fact⟩
  use b1, a1
  exact ⟨gridr_swap fact.1, ⟨fact.2.2, fact.2.1⟩⟩

theorem stable_first_one : stable [none] v := by
  intro a b gridrdy new_u new_v nu nv
  ...
  · use [none], [none]
    ...
    · rw [BraidMonoidInf.one_of_eq_mk_one nu.symm, BraidMonoidInf.one_of_eq_mk_one nv.symm]
      exact gridr.empty
    rw [(all_ones gridrdy rfl rfl).1, (all_ones gridrdy rfl rfl).2]
    exact ⟨rfl, rfl⟩
  rw [BraidMonoidInf.one_of_eq_mk_one nu.symm]
  unfold BraidMonoidInf.mk at nv
  rename_i c d e
  have Ha : a = 1 := (word_side_side _ _ _ gridrdy).1
  have Hb : b = of c * d := (word_side_side _ _ _ gridrdy).2
  ...
  | of x y _ =>
    ...
    use 1, y
    constructor
    · exact gridr_top_bottom_word _
    · rw [h]
      exact ⟨rfl, PresentedMonoid.sound (ConGen.Rel.of x y br)⟩
  | refl x =>
    use 1, x
    exact  ⟨gridr_top_bottom_word _, ⟨rfl, by rw [h]⟩⟩
  | symm one _ =>
    rename_i z _ _
    use 1, z
    constructor
    ...
    · rfl
    rw [h]
    exact PresentedMonoid.sound (ConGen.Rel.symm one)
  | trans one two _ =>
    rename_i o _ _
    rw [h]
    use 1, o
    exact
      ⟨gridr_top_bottom_word o, ⟨rfl, PresentedMonoid.sound (ConGen.Rel.trans one two)⟩⟩
  | mul one two _ _ =>
    rename_i n _ p _ _
    rw [h]
    use 1, n * p
    exact ⟨gridr_top_bottom_word (n * p), ⟨rfl, PresentedMonoid.sound (ConGen.Rel.mul one two)⟩⟩

theorem stable_second_one : stable a [none] := by
  ...
  · rename_i e f g
    use f, 1
    constructor
    · exact gridr_sides_word f
    constructor
    · rw [← Hc]
      exact PresentedMonoid.sound (ConGen.Rel.of e f g)
    rfl
  · rename_i e
    use e, 1
    exact ⟨gridr_sides_word _, ⟨by rw [Hc], rfl⟩⟩
  · rename_i e f g _
    use e, 1
    exact ⟨gridr_sides_word _, ⟨by rw [← Hc]; exact PresentedMonoid.sound (ConGen.Rel.symm g), rfl⟩⟩
  · rename_i e f g i j _ _
    use g, 1
    constructor
    · exact gridr_sides_word _
    constructor
    · rw [← Hc]
  ...
  · rename_i e f i j k l _ _
    ...
    constructor
    · rw [← Hc]
      exact PresentedMonoid.sound (ConGen.Rel.mul k l)
    rfl
...
theorem reg_helper (ih : ∀ (u v a b : FreeMonoid ℕ), n ≥ u.length + b.length → gridr u v a b →
    ∀ (u' v' : FreeMonoid ℕ), BraidMonoidInf.mk u = BraidMonoidInf.mk u' →
    BraidMonoidInf.mk v = BraidMonoidInf.mk v' → ∃ a' b', gridr u' v' a' b' ∧
    BraidMonoidInf.mk a = BraidMonoidInf.mk a' ∧ BraidMonoidInf.mk b = BraidMonoidInf.mk b')
    (br : braid_rels_m_inf f g) (gr : gridr e (i * f * j) c d) (len : n + 1 ≥ e.length + d.length) :
    ∃ a' b', gridr e (i * g * j) a' b' ∧
    BraidMonoidInf.mk c = BraidMonoidInf.mk a' ∧ BraidMonoidInf.mk d = BraidMonoidInf.mk b' := by
  rcases splittable_vertically_of_gridr gr _ _ rfl with ⟨u₁, d₄, d₃, first_gridr, gridr_right, d_is⟩
  have H_split1 := splittable_vertically_of_gridr first_gridr _ _ rfl
  rcases H_split1 with ⟨u, d₁, d₂, gridr_left, gridr_middle, d₄_is⟩
  induction u using FreeMonoid.inductionOn'
  · use 1, d₁ * g * d₃
    have H := word_side_side _ _ _ gridr_middle
    rw [H.1] at gridr_right
    rw [H.2] at d₄_is
    have H := word_side_side _ _ _ gridr_right
    rw [H.1] at gridr_right
    rw [H.1]
    constructor
    · exact gridr.horizontal (gridr.horizontal gridr_left (gridr_top_bottom_word g)) gridr_right
    constructor
    · rfl
    rw [d_is, d₄_is]
    apply PresentedMonoid.sound
  ...
  have H_split := splittable_horizontally_of_gridr gridr_middle _ _ rfl
  ...
  have H_len : n ≥ tail.length + d₂.length := by
    have two : e.length + d.length = (i * f * j).length + c.length := by
      have H := gridr_diag_length_eq gr
    ...
    have H4 : (f * j).length + c.length = (of head * tail).length + (d₂ * d₃).length := by
      rw [u₁_is] at gridr_right
      have H := gridr_diag_length_eq (gridr.horizontal
        (gridr.vertical gr_top_middle gr_bottom_middle) gridr_right)
      simp only [length_mul] at H
      simp only [length_mul]
      exact H.symm
  ...
  have H_len : n ≥ (a₁ * a₂).length + d₃.length := by
    have one : (a₁ * a₂).length + d₃.length = j.length + c.length := by
      have H := gridr_diag_length_eq gridr_right
      simp only [length_mul] at H
      simp only [length_mul]
      exact H
    rw [one]
  ...
  rcases ih (a₁ * a₂) j  c d₃ H_len gridr_right _ _ H_st rfl with ⟨c', d₃', right_fact⟩
  use c', d₁ * d₂' * d₃'
  constructor
  · exact gridr.horizontal (gridr.horizontal gridr_left
      (gridr.vertical top_middle_fact.1 bottom_middle_fact.1)) right_fact.1
  constructor
...
theorem symm_helper (ih : ∀ (u v a b : FreeMonoid ℕ), n ≥ u.length + b.length → gridr u v a b →
    ∀ (u' v' : FreeMonoid ℕ), BraidMonoidInf.mk u = BraidMonoidInf.mk u' →
    BraidMonoidInf.mk v = BraidMonoidInf.mk v' → ∃ a' b', gridr u' v' a' b' ∧
    BraidMonoidInf.mk a = BraidMonoidInf.mk a' ∧ BraidMonoidInf.mk b = BraidMonoidInf.mk b')
    (br : braid_rels_m_inf f g) (gr : gridr e (i * g * j) c d) (len : n + 1 ≥ e.length + d.length) :
    ∃ a' b', gridr e (i * f * j) a' b' ∧
    BraidMonoidInf.mk c = BraidMonoidInf.mk a' ∧ BraidMonoidInf.mk d = BraidMonoidInf.mk b' := by
  have H_split := splittable_vertically_of_gridr gr _ _ rfl
  rcases H_split with ⟨u₁, d₄, d₃, first_gridr, gridr_right, d_is⟩
  have H_split1 := splittable_vertically_of_gridr first_gridr _ _ rfl
  rcases H_split1 with ⟨u, d₁, d₂, gridr_left, gridr_middle, d₄_is⟩
  induction u using FreeMonoid.inductionOn'
  · use 1, d₁ * f * d₃
    ...
    rw [H.1]
    constructor
    · exact gridr.horizontal (gridr.horizontal gridr_left (gridr_top_bottom_word f)) gridr_right
    constructor
    · rfl
    rw [d_is, d₄_is]
...
-- a gridr is stable when only the second element moves
theorem stable_second (ih : ∀ (u v a b : FreeMonoid ℕ), n ≥ u.length + b.length → gridr u v a b →
    ∀ (u' v' : FreeMonoid ℕ), BraidMonoidInf.mk u = BraidMonoidInf.mk u' →
    BraidMonoidInf.mk v = BraidMonoidInf.mk v' → ∃ a' b', gridr u' v' a' b' ∧
    BraidMonoidInf.mk a = BraidMonoidInf.mk a' ∧ BraidMonoidInf.mk b = BraidMonoidInf.mk b')
    (b_is : BraidMonoidInf.mk f = BraidMonoidInf.mk i) :
    ∀ (d : FreeMonoid ℕ), n + 1 ≥ a.length + d.length →
    ∀ (c : FreeMonoid ℕ), gridr a f c d → ∃ a' b', gridr a i a' b' ∧
    BraidMonoidInf.mk c = BraidMonoidInf.mk a' ∧ BraidMonoidInf.mk d = BraidMonoidInf.mk b' := by
...
theorem stability (u v : FreeMonoid ℕ) : stable u v := by
  have H1 : ∀ t u v, ∀ a b, t >= u.length + b.length → gridr u v a b → ∀ u' v',
      BraidMonoidInf.mk u = BraidMonoidInf.mk u' →
      BraidMonoidInf.mk v = BraidMonoidInf.mk v' → ∃ a' b',
      gridr u' v' a' b' ∧ BraidMonoidInf.mk a = BraidMonoidInf.mk a' ∧
    ...
    | succ n ih =>
      ...
      · intro g i e f br b b_is d len c gr
        have easy_len : n + 1 ≥ b.length + c.length := by
          rw [← gridr_diag_length_eq gr]
          exact len
        rcases reg_helper ih br (gridr_swap gr) easy_len with ⟨a1, b1, swapped_gridr, da, cb⟩
        apply gridr_swap at swapped_gridr
        have easy_len2 : n + 1 ≥ (e * i * f).length + a1.length := by
          ...
          assumption
        rcases stable_second ih b_is a1 easy_len2 b1 swapped_gridr with ⟨a2, b2, second_fact⟩
        use a2, b2
      ...
      · intro _ _ g i br b b_is d len c gr
        have easy_len : n + 1 ≥ b.length + c.length := by
          rw [← gridr_diag_length_eq gr]
          exact len
        rcases symm_helper ih br (gridr_swap gr) easy_len with ⟨a1, b1, swapped_gridr, da, cb⟩
        apply gridr_swap at swapped_gridr
        rename_i x x2
      ...
      · intro ha1 hb1 hc1 ih b b_is d len c gr
        ...
        have H_len : n + 1 ≥ hb1.length + d₁.length := by
          ...
          rw [← gridr_diag_length_eq (gridr_swap first_fact.1), Hb, Hc, ← gridr_diag_length_eq gr]
          exact len
